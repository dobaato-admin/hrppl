...
function convertUnit(e, n) {
  ...
  const u = a$1.get(t);
  if (!u) return n;
  const i = u(n[4].value), o = [c.Dimension, "", n[2], n[3], { ...n[4], signCharacter: i < 0 ? "-" : void 0, type: Number.isInteger(i) ? a.Integer : a.Number, value: i }];
  return mutateUnit(o, e[4].unit), o;
}
function toCanonicalUnit(e) {
  if (!isTokenDimension(e)) return e;
  const n = toLowerCaseAZ(e[4].unit), t = T[n];
  if (n === t) return e;
  const r = Y.get(n);
  if (!r) return e;
  const a$1 = r.get(t);
  if (!a$1) return e;
  const u = a$1(e[4].value), i = [c.Dimension, "", e[2], e[3], { ...e[4], signCharacter: u < 0 ? "-" : void 0, type: Number.isInteger(u) ? a.Integer : a.Number, value: u }];
...
function addition(e, t) {
  ...
  return (isTokenNumber(r) && (isTokenDimension(a$1) || isTokenPercentage(a$1)) || isTokenNumber(a$1) && (isTokenDimension(r) || isTokenPercentage(r))) && t.onParseError?.(new ParseErrorWithComponentValues(y.UnexpectedAdditionOfDimensionOrPercentageWithNumber, e)), -1;
}
function division(e) {
  if (2 !== e.length) return -1;
...
function resolveGlobalsAndConstants(e, r) {
  for (let a$1 = 0; a$1 < e.length; a$1++) {
    ...
    const o = toLowerCaseAZ(i[4].value);
    switch (o) {
      case "e":
        e.splice(a$1, 1, new TokenNode([c.Number, Math.E.toString(), i[2], i[3], { value: Math.E, type: a.Number }]));
        break;
      case "pi":
        e.splice(a$1, 1, new TokenNode([c.Number, Math.PI.toString(), i[2], i[3], { value: Math.PI, type: a.Number }]));
        break;
      case "infinity":
        e.splice(a$1, 1, new TokenNode([c.Number, "infinity", i[2], i[3], { value: 1 / 0, type: a.Number }]));
        break;
      case "-infinity":
        e.splice(a$1, 1, new TokenNode([c.Number, "-infinity", i[2], i[3], { value: -1 / 0, type: a.Number }]));
        break;
      case "nan":
        e.splice(a$1, 1, new TokenNode([c.Number, "NaN", i[2], i[3], { value: Number.NaN, type: a.Number }]));
        break;
      default:
        if (r.has(o)) {
          const t = r.get(o);
...
function solveClamp(e, n, r, a2, u) {
  ...
  const l = convertUnit(i, a2.value);
...
function solveHypot(e, n, r) {
  ...
  const i = u.map((e2) => e2[4].value), o = Math.hypot(...i);
  return resultToCalculation(e, a2, o);
}
function solveMax(e, n, r) {
  ...
  const u = n.map((e2) => convertUnit(a2, e2.value));
  if (!arrayOfSameNumeric(u)) return -1;
  const i = u.map((e2) => e2[4].value), o = Math.max(...i);
  return resultToCalculation(e, a2, o);
}
function solveMin(e, n, r) {
  ...
  const u = n.map((e2) => convertUnit(a2, e2.value));
  if (!arrayOfSameNumeric(u)) return -1;
  const i = u.map((e2) => e2[4].value), o = Math.min(...i);
  return resultToCalculation(e, a2, o);
}
...
function snapAsBorderWidth(e, n, t) {
  ...
  const i = Math.ceil(u[4].value / a$1[4].value) * a$1[4].value;
  return u[4].value = i, resultToCalculation(e, n, convertUnit(n, u)[4].value);
}
function solveRound(e, n, t, r, a2) {
  ...
  if (0 === i[4].value) o = Number.NaN;
    ...
    case "down":
      o = u[4].value < 0 ? -1 / 0 : Object.is(-0, 0 * u[4].value) ? -0 : 0;
      break;
    case "up":
      o = u[4].value > 0 ? 1 / 0 : Object.is(0, 0 * u[4].value) ? 0 : -0;
      break;
    default:
      o = Object.is(0, 0 * u[4].value) ? 0 : -0;
  }
  else switch (n) {
    case "down":
    ...
    case "to-zero":
      ...
      break;
    default: {
      let t2 = Math.floor(u[4].value / i[4].value) * i[4].value, r2 = Math.ceil(u[4].value / i[4].value) * i[4].value;
      if (t2 > r2) {
        const e2 = t2;
        t2 = r2, r2 = e2;
      }
      const l = Math.abs(u[4].value - t2), c2 = Math.abs(u[4].value - r2);
      if (o = "line-width" === n && u[4].value >= 0 && (0 === r2 || 0 === t2) ? 0 !== r2 ? r2 : t2 : l === c2 ? r2 : l < c2 ? t2 : r2, "line-width" === n) {
        const n2 = solve(resultToCalculation(e, u, o), a2);
...
function solveTan(e, n) {
  ...
  const i = a2 / 90;
  return u = a2 % 90 == 0 && i % 2 != 0 ? i > 0 ? 1 / 0 : -1 / 0 : Math.tan(u), numberToCalculation(e, u);
}
function subtraction(e, t) {
  ...
  return (isTokenNumber(r) && (isTokenDimension(a$1) || isTokenPercentage(a$1)) || isTokenNumber(a$1) && (isTokenDimension(r) || isTokenPercentage(r))) && t.onParseError?.(new ParseErrorWithComponentValues(y.UnexpectedSubtractionOfDimensionOrPercentageWithNumber, e)), -1;
}
function solveLog(e, n) {
  ...
  if (2 === n.length) {
    ...
    return numberToCalculation(e, Math.log(a2[4].value) / Math.log(i[4].value));
  }
  return -1;
}
const _ = /^none$/i;
...
function solveRandom(e, n, t, r, a2, u) {
  ...
  if (!twoOfSameNumeric(i, o)) return -1;
  let l = null;
  if (a2 && (l = convertUnit(i, a2.value), !twoOfSameNumeric(i, l))) return -1;
  if (!Number.isFinite(i[4].value)) return resultToCalculation(e, i, Number.NaN);
  if (!Number.isFinite(o[4].value)) return resultToCalculation(e, i, Number.NaN);
  if (!Number.isFinite(o[4].value - i[4].value)) return resultToCalculation(e, i, Number.NaN);
  if (l && !Number.isFinite(l[4].value)) return resultToCalculation(e, i, i[4].value);
  const c2 = -1 === n.fixed ? sfc32(crc32([n.dashedIdent ? n.dashedIdent : `${u.randomCaching?.propertyName} ${u.randomCaching.propertyN++}`, n.elementShared ? "" : u.randomCaching.elementID, u.randomCaching.documentID].join(H))) : () => n.fixed;
  ...
  if (s > v && ([s, v] = [v, s]), l && (l[4].value <= 0 || Math.abs(s - v) / l[4].value > 1e10) && (l = null), l) {
    ...
    const a3 = c2();
    return resultToCalculation(e, i, Number(t2[Math.floor(t2.length * a3)].toFixed(5)));
  }
  const f = c2();
...
const J = /* @__PURE__ */ new Map([["abs", function abs(e, n, t) {
...
}], ["calc", calc$1], ["clamp", function clamp(r, a2, i) {
  const o = resolveGlobalsAndConstants([...r.value.filter((e) => !isWhiteSpaceOrCommentNode(e))], a2), c$1 = [], s = [], v = [];
  {
    let e = c$1;
  ...
  const f = isNone(c$1), p = isNone(v);
  if (f && p) return calc$1(calcWrapper(r, s), a2, i);
  const d = solve(calc$1(calcWrapper(r, s), a2, i), i);
  if (-1 === d) return -1;
  if (f) {
    const t = solve(calc$1(calcWrapper(r, v), a2, i), i);
    return -1 === t ? -1 : solveMin((C = r, g = d, D = t, new FunctionNode([c.Function, "min(", C.name[2], C.name[3], { value: "min" }], [c.CloseParen, ")", C.endToken[2], C.endToken[3], void 0], [g, new TokenNode([c.Comma, ",", ...sourceIndices(g), void 0]), D])), [d, t], i);
  }
  if (p) {
    const e = solve(calc$1(calcWrapper(r, c$1), a2, i), i);
    return -1 === e ? -1 : solveMax(maxWrapper(r, e, d), [e, d], i);
  }
  var C, g, D;
  const h = solve(calc$1(calcWrapper(r, c$1), a2, i), i);
  if (-1 === h) return -1;
  const N = solve(calc$1(calcWrapper(r, v), a2, i), i);
  if (-1 === N) return -1;
  ...
  if (-1 === r) return -1;
  const [a2, i] = r, o = variadicArguments(e, i, n, t);
  if (-1 === o) return -1;
  const [l, c2, s] = o;
  if (!l || !c2) return -1;
  ...
  const i = resolveGlobalsAndConstants([...e.value.filter((e2) => !isWhiteSpaceOrCommentNode(e2))], r);
  let o = "", l = false;
  const c$1 = [], s = [];
  {
    let e2 = c$1;
    for (let n = 0; n < i.length; n++) {
      const r2 = i[n];
      if (!o && 0 === c$1.length && 0 === s.length && isTokenNode(r2) && isTokenIdent(r2.value)) {
      ...
      if (isTokenNode(r2) && isTokenComma(r2.value)) {
        if (e2 === s) return -1;
        if (e2 === c$1 && o && 0 === c$1.length) continue;
        if (e2 === c$1) {
          l = true, e2 = s;
          continue;
        }
...
function twoCommaSeparatedArguments(e, n, r) {
  const a2 = resolveGlobalsAndConstants([...e.value.filter((e2) => !isWhiteSpaceOrCommentNode(e2))], n), i = [], o = [];
  {
    let e2 = i;
  ...
  if (-1 === l) return -1;
  const c2 = solve(calc$1(calcWrapper(e, o), n, r), r);
  return -1 === c2 ? -1 : [l, c2];
}
...
function variadicArguments(e, n, r, a2) {
  const i = resolveGlobalsAndConstants([...n.filter((e2) => !isWhiteSpaceOrCommentNode(e2))], r), o = [];
  {
    const n2 = [];
    let u = [];
...
function parseRandomValueSharing(e, n, r, a2) {
  const u = { isAuto: false, dashedIdent: "", fixed: -1, elementShared: false }, i = n[0];
  if (!isTokenNode(i) || !isTokenIdent(i.value)) return [u, n];
  for (let i2 = 0; i2 < n.length; i2++) {
    const o = n[i2];
    if (!isTokenNode(o)) return -1;
    if (isTokenComma(o.value)) return [u, n.slice(i2 + 1)];
    ...
    if ("element-shared" !== l) {
      if ("fixed" === l) {
        ...
        const t = n[i2];
        if (!t) return -1;
        const o2 = solve(calc$1(calcWrapper(e, [t]), r, a2), a2);
        if (-1 === o2) return -1;
        if (!isTokenNumber(o2.value)) return -1;
        if (o2.value[4].value < 0 || o2.value[4].value > 1) return -1;
        u.fixed = Math.max(0, Math.min(o2.value[4].value, 1 - 1e-9));
        continue;
      }
...
function replaceComponentValues(n, r) {
  for (let a2 = 0; a2 < n.length; a2++) {
    ...
    walk(i, (n2, a3) => {
      ...
      const i2 = r(n2.node);
      if (!i2) return;
      const o = [i2], l = n2.parent.value[a3 - 1];
      isTokenNode(l) && isTokenDelim(l.value) && ("-" === l.value[4].value || "+" === l.value[4].value) && o.splice(0, 0, new WhitespaceNode([[c.Whitespace, " ", ...sourceIndices(n2.node), void 0]]));
      const s = n2.parent.value[a3 + 1];
      !s || isWhiteSpaceOrCommentNode(s) || isTokenNode(s) && (isTokenComma(s.value) || isTokenColon(s.value) || isTokenSemicolon(s.value) || isTokenDelim(s.value) && "-" !== s.value[4].value && "+" !== s.value[4].value) || o.push(new WhitespaceNode([[c.Whitespace, " ", ...sourceIndices(n2.node), void 0]])), n2.parent.value.splice(a3, 1, ...o);
    });
  }
  return n;
}
