...
export const seedStripePrices = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: existing } = await supabaseAdmin
      .from('subscription_plans').select('id, code, stripe_price_id').in('code', specs.map((s) => s.code));
    const out: Array<{ code: string; price_id: string }> = [];
    for (const spec of specs) {
      const row = existing?.find((r: any) => r.code === spec.code);
      if (row?.stripe_price_id) { out.push({ code: spec.code, price_id: row.stripe_price_id }); continue; }
      const product = await stripe.products.create({ name: spec.name, metadata: { plan_code: spec.code } });
      const price = await stripe.prices.create({
        product: product.id, currency: 'usd', unit_amount: spec.amountCents,
        recurring: { interval: 'month', usage_type: 'metered', aggregate_usage: 'last_during_period' } as any,
        metadata: { plan_code: spec.code, basis: 'net_active_employees_per_calendar_month' },
      });
      await supabaseAdmin.from('subscription_plans').update({ stripe_price_id: price.id }).eq('code', spec.code);
...
export const resolveBillingAlert = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: before } = await supabaseAdmin
      .from('billing_admin_alerts').select('id, status, tenant_id').eq('id', data.id).single();
    const { data: after } = await supabaseAdmin.from('billing_admin_alerts')
    ...
    await logBillingOpsAudit({
      ...
      target_type: 'alert', target_id: data.id,
...
export const retryBillingAlert = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: alert } = await supabaseAdmin
      .from('billing_admin_alerts').select('*').eq('id', data.id).single();
    if (!alert) throw new Error('Alert not found');

    // Idempotency layer 1: already resolved → no-op.
    if ((alert as any).status === 'resolved') {
      await logBillingOpsAudit({
        actor_user_id: userId, actor_email: await actorEmail(supabase, userId),
        action: 'alert.retry.noop', target_type: 'alert', target_id: data.id,
        tenant_id: (alert as any).tenant_id, before: alert, after: alert,
    ...
    if (
      ...
      const { runMonthlyBillingForTenant, runMonthlyBillingForAllTenants } = await import('@/lib/billing.server');
      if ((alert as any).tenant_id && ctx.year && ctx.month) {
        result = await runMonthlyBillingForTenant((alert as any).tenant_id, ctx.year, ctx.month, false);
      } else if (ctx.year && ctx.month) {
        result = await runMonthlyBillingForAllTenants(ctx.year, ctx.month);
      }
    } else if ((alert as any).alert_type === 'reconciliation_run_failed' || (alert as any).alert_type === 'reconciliation_discrepancy' || (alert as any).alert_type === 'reconciliation_stripe_read_failed') {
      ...
      const { getStripe } = await import('@/lib/stripe.server');
      const stripe = getStripe();
      result = await stripe.invoices.pay(ctx.invoice_id, undefined, {
        idempotencyKey: `invoice_pay:${ctx.invoice_id}`,
      }).catch((e: any) => ({ error: e?.message }));
    }

    const ok = !result?.error && (Array.isArray(result) ? !result.some((r: any) => r.status === 'failed') : result?.status !== 'failed');
    let after = before;
    if (ok) {
      const { data: resolved } = await supabaseAdmin.from('billing_admin_alerts')
        .update({ status: 'resolved', resolved_at: new Date().toISOString(), resolved_by: userId, next_retry_at: null })
    ...
    await logBillingOpsAudit({
      ...
      before, after,
      metadata: { lockKey, alert_type: (alert as any).alert_type, result_summary: summarize(result) },
    });
...
function summarize(r: any): any {
  if (!r) return null;
  if (Array.isArray(r)) return { count: r.length, failed: r.filter((x: any) => x.status === 'failed').length };
  ...
  .handler(async ({ context }) => {
    ...
    return {
      items: (subs ?? []).map((s: any) => ({
        ...
        failed_invoices: (invoicesByTenant[s.tenant_id] ?? []).filter((i) =>
          i.status === 'uncollectible' || i.status === 'open' || (i.attempt_count ?? 0) > 1
        ).length,
      })),
    };
  });
...
export const exportBillingForMonth = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const rows = (snaps ?? []).map((s: any) => ({
      ...
      reconciliation_delta: (reconByTenant[s.tenant_id]?.base_delta ?? 0)
    ...
    if (data.format === 'csv') {
      const headers = Object.keys(rows[0] ?? {
        tenant_id: '', tenant_name: '', country: '', period: '',
        net_employees: 0, joined: 0, left: 0, base_units: 0, addon_units: 0,
        trial_applied: false, status: '', stripe_base_usage_id: '', stripe_addon_usage_id: '',
        reported_at: '', reconciled_reported_base: '', reconciled_reported_addon: '',
        reconciliation_delta: 0, has_discrepancy: false, error: '',
      });
      const csv = [
        headers.join(','),
      ...
      return {
        format: 'csv', filename: `billing-${data.year}-${String(data.month).padStart(2, '0')}.csv`,
        mime: 'text/csv', content: csv,
      };
    }
...
${rows.map((r: any) => `<tr class="${r.has_discrepancy ? 'bad' : ''}">
...
<p style="margin-top:16px;font-size:10px;color:#64748b">Generated ${new Date().toISOString()} — print this page to PDF for archive.</p>
      ...
      return {
        format: 'pdf', filename: `billing-${data.year}-${String(data.month).padStart(2, '0')}.html`,
...
export const scheduleTenantPlanChange = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: newPlan } = await supabaseAdmin
      .from('subscription_plans').select('*').eq('code', data.newPlanCode).single();
    if (!newPlan?.stripe_price_id) throw new Error('New plan has no Stripe price configured');

    const { data: sub } = await supabaseAdmin
      .from('tenant_subscriptions').select('*').eq('tenant_id', data.tenantId).single();
...
export const listBillingOpsAudit = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) => z.object({
...
export const listDiscrepancies = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: snaps } = tenantIds.length ? await supabaseAdmin
      .from('tenant_billing_snapshots')
      .select('tenant_id, period_year, period_month, net_employees, joined_count, left_count, base_units, addon_units, trial_applied, plan_change_prorated, stripe_base_usage_id, stripe_addon_usage_id, reported_at, status, error')
      .in('tenant_id', tenantIds) : { data: [] as any[] };
    const snapBy: Record<string, any> = {};
    for (const s of snaps ?? []) snapBy[`${(s as any).tenant_id}|${(s as any).period_year}|${(s as any).period_month}`] = s;
...
export const exportDiscrepanciesCsv = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const headers = [
      'tenant_id', 'tenant_name', 'country', 'period',
      'computed_base', 'reported_base', 'base_delta',
      'computed_addon', 'reported_addon', 'addon_delta',
      'has_discrepancy', 'notes', 'logged_at',
    ];
    const lines = [headers.join(',')];
    for (const r of rows ?? []) {
      const row: any = r;
      lines.push([
        row.tenant_id, row.tenants?.name ?? '', row.tenants?.country_code ?? '',
        `${row.period_year}-${String(row.period_month).padStart(2, '0')}`,
        row.computed_base, row.reported_base, row.base_delta,
        row.computed_addon, row.reported_addon, row.addon_delta,
        row.has_discrepancy, row.notes ?? '', row.created_at,
      ].map(csvEscape).join(','));
    }
    const period = data.year && data.month ? `-${data.year}-${String(data.month).padStart(2, '0')}` : '';
    return {
      format: 'csv', mime: 'text/csv',
      filename: `discrepancies${period}.csv`, content: lines.join('\n'),
    };
  });
...
export const listTenantsLite = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ context }) => {
    ...
    const { data } = await supabaseAdmin
      .from('tenants').select('id, name, country_code').order('name');
    return { items: data ?? [] };
  });
...
export const previewInvoiceImpact = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const result = await previewBillingForTenant({
      tenantId: data.tenantId, year: data.year, month: data.month,
    ...
    await logBillingOpsAudit({
      actor_user_id: userId, actor_email: await actorEmail(supabase, userId),
      action: 'preview.run', target_type: 'tenant', target_id: data.tenantId,
      tenant_id: data.tenantId, after: result,
      metadata: { year: data.year, month: data.month, overrides: {
        plan: data.overridePlanCode, planChangeDate: data.overridePlanChangeDate,
        trialEndsAt: data.overrideTrialEndsAt, addonAU: data.overrideAddonAU,
      } },
    });
    return result;
  });
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.id) {
      const { data: r, error } = await supabaseAdmin
        .from('billing_alert_suppressions').update(payload).eq('id', data.id).select('*').single();
      if (error) throw new Error(error.message);
      row = r;
    } else {
      const { data: r, error } = await supabaseAdmin
        .from('billing_alert_suppressions').insert(payload).select('*').single();
      if (error) throw new Error(error.message);
      row = r;
    }
    await logBillingOpsAudit({
      ...
      target_type: 'suppression', target_id: row.id,
      tenant_id: row.tenant_id, after: row,
    });
...
export const deleteAlertSuppression = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { logBillingOpsAudit } = await import('@/lib/billing-alerts.server');
    const { data: before } = await supabaseAdmin
      .from('billing_alert_suppressions').select('*').eq('id', data.id).single();
    await supabaseAdmin.from('billing_alert_suppressions').delete().eq('id', data.id);
    await logBillingOpsAudit({
      actor_user_id: userId, actor_email: await actorEmail(supabase, userId),
      action: 'suppression.delete', target_type: 'suppression', target_id: data.id,
      tenant_id: (before as any)?.tenant_id ?? null, before,
    });
    return { ok: true };
  });
...
export const listRetryPolicies = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ context }) => {
    ...
    const { supabaseAdmin } = await import('@/integrations/supabase/client.server');
    const { data } = await supabaseAdmin
      .from('billing_alert_retry_policies').select('*').order('alert_type');
    return { items: data ?? [] };
  });
...
export const upsertRetryPolicy = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { logBillingOpsAudit } = await import('@/lib/billing-alerts.server');
    const { data: before } = await supabaseAdmin
      .from('billing_alert_retry_policies').select('*').eq('alert_type', data.alert_type).maybeSingle();
    const { data: row, error } = await supabaseAdmin
      .from('billing_alert_retry_policies').upsert(data, { onConflict: 'alert_type' }).select('*').single();
    if (error) throw new Error(error.message);
    await logBillingOpsAudit({
      actor_user_id: userId, actor_email: await actorEmail(supabase, userId),
      action: 'retry_policy.update', target_type: 'retry_policy', target_id: data.alert_type,
      before, after: row,
    });
    return { ok: true, item: row };
  });
...
export const exportBillingOpsAuditCsv = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const r of rows ?? []) {
      const row: any = r;
      lines.push([
        row.created_at, row.actor_email ?? '', row.action,
        row.tenants?.name ?? '', row.target_type ?? '', row.target_id ?? '',
...
export async function processDueAutoRetries(limit = 25): Promise<{ processed: number; ok: number; failed: number }> {
  const { supabaseAdmin } = await import('@/integrations/supabase/client.server');
  const { getRetryPolicy, computeNextRetryAt, logBillingOpsAudit } = await import('@/lib/billing-alerts.server');
  ...
  for (const alert of (due as any[]) ?? []) {
    ...
    if (!policy?.enabled) {
      await supabaseAdmin.from('billing_admin_alerts').update({ next_retry_at: null }).eq('id', alert.id);
      continue;
    }
    ...
    const success = !result?.error && (Array.isArray(result)
    ...
    await logBillingOpsAudit({
      ...
      target_type: 'alert', target_id: alert.id, tenant_id: alert.tenant_id,
