// Industry-aware KPI / KRA / Competency / 360 review preset templates.
// Admins pick an industry + role; presets pre-populate and stay customizable.
// Education Agent presets are derived from the EEVS JD & KPI matrix (Jan 2026).
...
export interface ReviewSchedule {
  /** Cadence: how often the item should be reviewed. */
  type: "monthly" | "quarterly" | "half_yearly" | "annual" | "custom";
  /** Specific periods within a year. e.g. ["Q1","Q3"] or ["Jan","Apr","Jul","Oct"]. */
  periods?: string[];
  /** Optional first review date (YYYY-MM-DD) for "custom" schedules. */
  startDate?: string;
}

export interface PresetCompetency {
  id: string;
  label: string;
  description?: string;
  type: CompetencyType;
  ...
  /** Minimum number of evidence attachments the reviewer must provide. */
  minEvidenceCount?: number;
  /** Evidence types the reviewer MUST include (subset of evidenceTypes). */
  requiredEvidenceTypes?: EvidenceType[];
  /** Legacy free-text review period hint. */
  reviewPeriod?: string;
  /** Structured schedule — surfaces on the employee review timeline. */
  schedule?: ReviewSchedule;
}

export type TemplateKind = "kpi" | "kra" | "competency" | "mixed" | "360";
...
const id = (s: string) => s.replace(/[^a-z0-9]+/gi, "_").slice(0, 60).toLowerCase();
const num = (label: string, opts: Partial<PresetCompetency> = {}): PresetCompetency => ({
  id: id(label), label, type: "number", required: true, evidenceEnabled: true,
  evidenceTypes: ["document", "url"], ...opts,
});
const pct = (label: string, opts: Partial<PresetCompetency> = {}): PresetCompetency => ({
  id: id(label), label, type: "percentage", required: true, min: 0, max: 100, unit: "%",
  evidenceEnabled: true, evidenceTypes: ["document"], ...opts,
});
const rate = (label: string, opts: Partial<PresetCompetency> = {}): PresetCompetency => ({
  id: id(label), label, type: "rating", required: true, ...opts,
});
const yn = (label: string, opts: Partial<PresetCompetency> = {}): PresetCompetency => ({
  id: id(label), label, type: "yes_no", required: true, yesLabel: "Met", noLabel: "Not met",
  evidenceEnabled: true, evidenceTypes: ["document", "url"], ...opts,
});
const txt = (label: string, opts: Partial<PresetCompetency> = {}): PresetCompetency => ({
  id: id(label), label, type: "text", required: false, ...opts,
});
const cur = (label: string, opts: Partial<PresetCompetency> = {}): PresetCompetency => ({
  id: id(label), label, type: "currency", required: true, unit: "AUD",
  evidenceEnabled: true, evidenceTypes: ["document"], ...opts,
});

// ---------------------------------------------------------------------------
// EDUCATION AGENT — derived from EEVS JD & KPI matrix (Jan 2026)
// ---------------------------------------------------------------------------

const EDU_AGENT_PRESETS: ReviewPreset[] = [
  // ────────────── Operations Head / Branch Manager ──────────────
  {
    ...
    scaleMin: 1, scaleMax: 5, scaleLabels: ["Below", "On Target", "Exceeds"],
    competencies: [
  ...
  },
  {
    ...
    scaleMin: 1, scaleMax: 5, scaleLabels: ["Unsatisfactory", "Outstanding"],
    competencies: [
  ...
  // ────────────── GM Sales (NSW) ──────────────
  {
    ...
    scaleMin: 1, scaleMax: 5, scaleLabels: ["Below", "Exceeds"],
    competencies: [
  ...
  // ────────────── 360° — Education Agent specific ──────────────
  {
    ...
    scaleMin: 1, scaleMax: 5, scaleLabels: ["Strongly Disagree", "Strongly Agree"],
    competencies: [
...
// ---------------------------------------------------------------------------
// Generic cross-industry presets (kept short — admins can extend)
// ---------------------------------------------------------------------------

const GENERIC_PRESETS: ReviewPreset[] = [
  {
    key: "generic_kpi",
...
export function getPresetsByIndustry(industry: string): ReviewPreset[] {
  return REVIEW_PRESETS.filter((p) => p.industry === industry);
}
