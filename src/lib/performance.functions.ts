...
export const createReviewCycle = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabase.from("review_cycles").insert({
      tenant_id: prof.tenant_id, name: data.name, period_start: data.periodStart,
      period_end: data.periodEnd, status: "draft", created_by: userId,
    }).select("id").single();
...
export const activateReviewCycle = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: cycle } = await admin.from("review_cycles").select("*").eq("id", data.cycleId).maybeSingle();
    ...
    if ((cycle as any).template_id && templateVersion == null) {
      const { data: tpl } = await admin.from("review_templates" as any).select("version").eq("id", (cycle as any).template_id).maybeSingle();
      templateVersion = (tpl as any)?.version ?? 1;
    }
    await admin.from("review_cycles").update({ status: "active", template_version: templateVersion } as any).eq("id", cycle.id);
    // Create a review row per active employee in tenant
    const { data: emps } = await admin.from("employees").select("id,manager_id").eq("tenant_id", cycle.tenant_id).eq("status", "active");
    if (emps?.length) {
      const rows = emps.map((e: any) => ({
        tenant_id: cycle.tenant_id, employee_id: e.id, cycle_id: cycle.id,
        reviewer_id: e.manager_id, status: "draft" as const,
        template_id: (cycle as any).template_id ?? null,
        template_version: templateVersion,
      }));
      await admin.from("performance_reviews").upsert(rows as any, { onConflict: "cycle_id,employee_id" });
    }
    return { ok: true };
  });
...
export const updateReviewCycleTemplate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.templateId) {
      const { data: tpl } = await supabase.from("review_templates" as any).select("version").eq("id", data.templateId).maybeSingle();
...
export const closeReviewCycle = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const roles = await getRoles(supabase, userId);
    if (!roles.some((r) => ["org_admin", "super_admin"].includes(r))) throw new Error("Not authorized");
    const { error } = await supabase.from("review_cycles").update({ status: "closed" }).eq("id", data.cycleId);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const updateReviewCycleReminders = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const roles = await getRoles(supabase, userId);
    if (!roles.some((r) => ["org_admin", "super_admin"].includes(r))) throw new Error("Not authorized");
    const { error } = await supabase.from("review_cycles").update({
...
export const previewReviewReminderSchedule = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const admin = await loadAdmin();
    const { data: cycle } = await admin.from("review_cycles")
      .select("id,tenant_id,name,period_start,reminders_enabled,reminder_interval_days,reminder_start_offset_days,reminder_business_days_only,reminder_max_count")
      .eq("id", data.cycleId).maybeSingle();
    if (!cycle) throw new Error("Cycle not found");
    ...
    const { data: tenant } = await admin.from("tenants").select("country_code").eq("id", (cycle as any).tenant_id).maybeSingle();
    const countryCode: string | null = (tenant as any)?.country_code ?? null;

    const holidays: { date: string; name: string; recurring: boolean }[] = [];
    if (countryCode) {
      const { data: hs } = await admin.from("public_holidays")
        .select("holiday_date,name,is_recurring").eq("country_code", countryCode);
    ...
    type Day = { date: string; status: "scheduled" | "weekend" | "holiday" | "before-start" | "disabled" | "max-reached"; reason?: string };
    const days: Day[] = [];
    ...
    for (let i = 0; i < data.horizonDays; i += 1) {
      ...
      if (!cfg.remindersEnabled) { days.push({ date: iso, status: "disabled" }); continue; }
      if (d.getTime() < firstEligible.getTime()) { days.push({ date: iso, status: "before-start" }); continue; }
      const dow = d.getUTCDay();
      if (cfg.businessDaysOnly && (dow === 0 || dow === 6)) { days.push({ date: iso, status: "weekend" }); continue; }
      if (cfg.businessDaysOnly) {
        const h = holidayOn(iso);
        if (h) { days.push({ date: iso, status: "holiday", reason: h }); continue; }
      }
      if (cfg.maxCount != null && scheduledCount >= cfg.maxCount) {
        days.push({ date: iso, status: "max-reached" }); continue;
      }
    ...
    return {
      cycle: { id: (cycle as any).id, name: (cycle as any).name, period_start: (cycle as any).period_start },
      countryCode,
      config: cfg,
      days,
      scheduledCount,
...
export const upsertGoal = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const payload: any = {
      tenant_id: emp.tenant_id, employee_id: emp.id, cycle_id: data.cycleId ?? null,
      title: data.title, description: data.description ?? null,
      weight: data.weight, progress: data.progress, status: data.status,
    ...
    const { data: row, error } = await supabase.from("performance_goals").insert(payload).select("id").single();
    if (error) throw new Error(error.message);
    return { ok: true, id: row.id };
  });
...
export const submitSelfReview = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const patch: any = {
      self_rating: Math.round(data.selfRating), self_comments: data.selfComments ?? null,
      status: "self_submitted",
    };
    if (data.responses) patch.self_responses = data.responses;
    const { error } = await supabase.from("performance_reviews").update(patch).eq("id", data.reviewId);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const submitManagerReview = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const payload: any = {
      manager_rating: Math.round(data.managerRating), manager_comments: data.managerComments ?? null,
    ...
    const { error } = await supabase.from("performance_reviews").update(payload).eq("id", data.reviewId);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const addReviewFeedback = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d) => z.object({
    reviewId: z.string().uuid(), kind: z.enum(["peer", "upward"]).default("peer"),
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await supabase.from("review_feedback").insert({
      tenant_id: prof.tenant_id, review_id: data.reviewId, author_id: userId,
...
const competencySchema = z.object({
  ...
  type: z.enum(["rating", "text", "number", "range", "yes_no", "scale", "percentage", "currency"]).default("rating"),
...
export const upsertReviewTemplate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.isDefault) {
      await supabase.from("review_templates" as any).update({ is_default: false }).eq("tenant_id", prof.tenant_id);
    }

    if (data.id && data.bumpVersion) {
      const { data: existing } = await supabase.from("review_templates" as any).select("*").eq("id", data.id).maybeSingle();
      if (!existing) throw new Error("Template not found");
      const e: any = existing;
      await supabase.from("review_templates" as any).update({ is_current: false }).eq("id", data.id);
      const { data: row, error } = await supabase.from("review_templates" as any).insert({
        tenant_id: prof.tenant_id, name: data.name, description: data.description ?? null,
        industry: data.industry ?? null, kind: data.kind,
        scale_min: data.scaleMin, scale_max: data.scaleMax, scale_labels: data.scaleLabels,
        competencies: data.competencies, is_default: data.isDefault,
        version: (e.version ?? 1) + 1, parent_template_id: e.parent_template_id ?? data.id,
        is_current: true, change_note: data.changeNote ?? null,
        created_by: e.created_by, updated_by: userId,
    ...
    if (data.id) {
      const { error } = await supabase.from("review_templates" as any).update({
        name: data.name, description: data.description ?? null,
        industry: data.industry ?? null, kind: data.kind,
        scale_min: data.scaleMin, scale_max: data.scaleMax, scale_labels: data.scaleLabels,
        competencies: data.competencies, is_default: data.isDefault,
        change_note: data.changeNote ?? null, updated_by: userId,
    ...
    const { data: row, error } = await supabase.from("review_templates" as any).insert({
      ...
      scale_min: data.scaleMin, scale_max: data.scaleMax, scale_labels: data.scaleLabels,
      competencies: data.competencies, is_default: data.isDefault,
      created_by: userId, updated_by: userId, change_note: data.changeNote ?? null,
    } as any).select("id").single();
    if (error) throw new Error(error.message);
    return { ok: true, id: (row as any).id };
  });
...
export const deleteReviewTemplate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const roles = await getRoles(supabase, userId);
    if (!roles.some((r) => ["org_admin", "super_admin"].includes(r))) throw new Error("Not authorized");
    const { error } = await supabase.from("review_templates" as any).delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const getCycleProgress = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase } = context;
    const { data: rows, error } = await supabase.from("performance_reviews")
      .select("status,manager_rating,calibrated_rating,self_rating").eq("cycle_id", data.cycleId);
    if (error) throw new Error(error.message);
    const all = (rows ?? []) as any[];
    const counts: Record<string, number> = { draft: 0, self_submitted: 0, manager_submitted: 0, finalized: 0, acknowledged: 0 };
    let ratingSum = 0, ratingN = 0;
    for (const r of all) {
      counts[r.status] = (counts[r.status] ?? 0) + 1;
      const finalR = r.calibrated_rating ?? r.manager_rating;
      if (finalR != null) { ratingSum += Number(finalR); ratingN += 1; }
    }
...
export const getReviewAuditTrail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const roles = await getRoles(supabase, userId);
    if (!roles.some((r) => ["manager", "org_admin", "super_admin"].includes(r))) throw new Error("Not authorized");
    let q = supabase.from("audit_log").select("*").eq("entity_type", "performance_review")
