// Minimal, safe markdown -> HTML for knowledge articles.
// Supports headings, paragraphs, lists (ordered/unordered), bold/italic, inline code, code fences, links.
// Escapes HTML in source first, then converts.
...
// File extensions that should be force-downloaded rather than opened inline.
// Browsers otherwise hand .docx/.xlsx/etc to a PDF/Office plugin which fails
...
function inline(s: string): string {
  ...
  s = s.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (_m, text, url) => {
    ...
    const attrs = isFile
      ? `download="${esc(filename)}" rel="noopener noreferrer"`
...
export function renderMarkdown(src: string): string {
  ...
  for (const rawLine of lines) {
    ...
    if (ul) {
      flushPara();
      if (inOl) { out.push("</ol>"); inOl = false; }
      if (!inUl) { out.push('<ul class="my-3 ml-6 list-disc space-y-1">'); inUl = true; }
    ...
    if (ol) {
      flushPara();
      if (inUl) { out.push("</ul>"); inUl = false; }
      if (!inOl) { out.push('<ol class="my-3 ml-6 list-decimal space-y-1">'); inOl = true; }
