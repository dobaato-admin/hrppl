          ...
          <Table>
            ...
            <TableBody>
              ...
              {!isLoading && leads.length === 0 && (
                <TableRow><TableCell colSpan={8} className="text-center text-sm text-muted-foreground py-8">No leads yet.</TableCell></TableRow>
              )}
              {leads.map((l) => (
                <TableRow key={l.id}>
                  ...
                  <TableCell><a href={`mailto:${l.work_email}`} className="text-primary hover:underline">{l.work_email}</a></TableCell>
                  <TableCell>{l.company}</TableCell>
                  <TableCell>{l.country ?? "—"}</TableCell>
                  <TableCell>
                    <Badge variant="outline">{l.company_size ?? "—"}</Badge>
                    {l.employee_count != null && (
                      <div className="text-xs text-muted-foreground mt-1">{l.employee_count} emp</div>
                    )}
                  </TableCell>
                  ...
                  <TableCell>
                    <Select
                      ...
                      onValueChange={(v) => mut.mutate({ id: l.id as string, status: v as (typeof STATUSES)[number] })}
                    >
