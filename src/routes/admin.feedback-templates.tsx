...
interface HistoryEntry extends Template {
...
function FeedbackTemplatesAdmin() {
  ...
  const canAccess = can("org.feedbackTemplates", roles);
  useEffect(() => {
  ...
  async function archive(id: string) {
    if (
      !confirm(
        "Archive this template? Existing feedback keeps its template, and you can restore it later.",
      )
    )
      return;
    setBusy(true);
    try {
      await fnArchive({ data: { id } });
      ...
      <main className="min-h-screen bg-background">
          ...
          <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Configure questions and rating scales used when colleagues give feedback.
              </p>
            </div>
            <Link to="/org">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  One template can be marked default and pre-selected when colleagues request
                  feedback.
                </CardDescription>
              </div>
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  data-testid="start-from-recommended"
                >
                  Start from recommended
                </Button>
                <Button size="sm" onClick={openCreate} data-testid="start-from-scratch">
                  Start from scratch
                </Button>
              </div>
            </CardHeader>
                  ...
                  {templates.map((t) => (
                    <TableRow key={t.id}>
                      ...
                      <TableCell className="text-right space-x-1">
                        <Button size="sm" variant="outline" onClick={() => openHistory(t)}>
                          History
                        </Button>
                ...
                <div>
                  <Label>
                    ...
                    <span className="text-xs text-muted-foreground">
                      (required — saved as new version v{(editing.version ?? 1) + 1})
                    </span>
                  </Label>
                  <Input
                    placeholder="e.g. Reworded leadership question, raised scale to 1–7"
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="space-x-2">
                    <Button size="sm" variant="outline" onClick={() => addQ("rating")}>
                      + Rating
                    </Button>
                  ...
                  <div key={q.id} className="rounded-md border p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Button size="sm" variant="ghost" onClick={() => removeQ(idx)}>
                          Remove
                        </Button>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <div>
                          ...
                          <Input
                            ...
                            max={9}
                            value={q.scaleMin ?? 1}
                            onChange={(e) => updateQ(idx, { scaleMin: Number(e.target.value) })}
                          />
                        </div>
                        <div>
                          ...
                          <Input
                            ...
                            max={10}
                            value={q.scaleMax ?? 5}
                            onChange={(e) => updateQ(idx, { scaleMax: Number(e.target.value) })}
                          />
                        </div>
                        <div>
        ...
        <Dialog open={presetOpen} onOpenChange={setPresetOpen}>
          <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              <p className="text-xs text-muted-foreground">
                Pick a starting point — you can rename it, add or remove questions, and adjust the
                rating scale before saving.
              </p>
        ...
        <Dialog open={archivedOpen} onOpenChange={setArchivedOpen}>
          <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
              ...
              {archived.map((t) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      v{t.version} · {(t.questions ?? []).length} questions
                    </p>
                  </div>
                  <Button
                    size="sm"
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setArchivedOpen(false)}>
                Close
              </Button>
            </DialogFooter>
          </DialogContent>
