...
function OvertimeWizard() {
  ...
  return (
    <AppShell title="Overtime Setup Wizard" subtitle="Step 4 of admin setup — required before inviting employees">
      <div className="mx-auto max-w-3xl space-y-6 p-6">
        <Card>
          <CardHeader>
            <CardTitle>Overtime & penalty rates</CardTitle>
            <CardDescription>
              Configure at least one overtime or penalty rate so timesheets and payroll can price hours correctly.
              Employee invitations remain blocked until at least one rate is active.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex items-center gap-2 rounded-md border p-3">
              ...
              <div className="text-sm">
                ...
                <div className="text-muted-foreground">{readinessQ.data?.rateCount ?? 0} rate(s) configured</div>
              </div>
              {done && (
                <Button size="sm" className="ml-auto" onClick={() => navigate({ to: "/org/invitations" })}>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Add a rate {done && <Badge variant="secondary" className="ml-2">Optional — add more</Badge>}</CardTitle>
            <CardDescription>Quick-add form. Manage the full list on the Overtime rates admin page.</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <p className="text-center text-xs text-muted-foreground">
          ...
          <Link to="/admin/overtime-rates" className="underline">Overtime rates</Link> admin.
        </p>
      </div>
    </AppShell>
  );
