...
const STEPS: { key: StepKey; title: string; blurb: string }[] = [
  { key: "payItems", title: "Pay items", blurb: "Define at least one active earning/deduction so payslips can render." },
  { key: "payDates", title: "Pay dates", blurb: "Set the pay period and standard hours that drive every payroll run." },
  { key: "overtimeRates", title: "Overtime & penalty rates", blurb: "Add at least one rate so overtime can be priced correctly." },
  { key: "currency", title: "Currency", blurb: "Confirm the ISO 4217 currency for payroll, FX, and invoicing." },
];
...
function WizardPage() {
  ...
  const steps = readinessQ.data?.steps ?? { payItems: false, payDates: false, overtimeRates: false, currency: false };
      ...
      <div className="mx-auto max-w-4xl space-y-6 p-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            {active.key === "payDates" && (
              <PayDatesStep
                existing={setupQ.data?.settings ?? null}
                onSaved={refresh}
              />
            )}
            {active.key === "overtimeRates" && (
              <OvertimeStep
                hasRates={steps.overtimeRates}
                onSaved={refresh}
              />
            )}
            {active.key === "currency" && (
        ...
        <p className="text-center text-xs text-muted-foreground">
          ...
          <Link to="/admin/payroll-setup" className="underline">Payroll Setup admin</Link>.
        </p>
      </div>
    </AppShell>
  );
...
function PayItemsStep({ hasItems, components, onSaved }: { hasItems: boolean; components: any[]; onSaved: () => void }) {
  const save = useServerFn(upsertPayrollComponent);
  const [code, setCode] = useState("BASIC");
  const [label, setLabel] = useState("Basic salary");
  const [kind, setKind] = useState<"allowance" | "deduction" | "pf" | "tax" | "retirement" | "other">("allowance");
  ...
  const submit = async () => {
    setBusy(true);
    try {
      await save({ data: { code, label, kind, calc_type: calc, rate: Number(rate) || 0, is_active: true, show_on_payslip: true } });
    ...
    <div className="space-y-4">
      {hasItems ? (
        <div className="rounded-md border bg-muted/40 p-3 text-sm">
          {components.length} pay item{components.length === 1 ? "" : "s"} configured. Add another below or move to the next step.
        </div>
      ) : (
      ...
      <Button onClick={submit} disabled={busy || !code || !label}>{busy ? "Saving…" : "Add pay item"}</Button>
    </div>
  );
}
...
function PayDatesStep({ existing, onSaved }: { existing: any; onSaved: () => void }) {
  ...
  const [period, setPeriod] = useState<"weekly" | "fortnightly" | "semimonthly" | "monthly">(existing?.pay_period ?? "monthly");
  const [hoursPerDay, setHoursPerDay] = useState<string>(String(existing?.standard_hours_per_day ?? 8));
  const [daysPerWeek, setDaysPerWeek] = useState<string>(String(existing?.standard_days_per_week ?? 5));
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div><Label>Standard hours/day</Label><Input type="number" value={hoursPerDay} onChange={(e) => setHoursPerDay(e.target.value)} /></div>
        <div><Label>Standard days/week</Label><Input type="number" value={daysPerWeek} onChange={(e) => setDaysPerWeek(e.target.value)} /></div>
        <div><Label>Meal break (mins)</Label><Input type="number" value={meal} onChange={(e) => setMeal(e.target.value)} /></div>
        <div><Label>Rest break (mins)</Label><Input type="number" value={rest} onChange={(e) => setRest(e.target.value)} /></div>
      </div>
      <Button onClick={submit} disabled={busy}>{busy ? "Saving…" : "Save pay schedule"}</Button>
    </div>
  );
}
...
function OvertimeStep({ hasRates, onSaved }: { hasRates: boolean; onSaved: () => void }) {
  ...
  const submit = async () => {
    setBusy(true);
    try {
      await save({ data: { code, name, applies_to: appliesTo, rate_multiplier: Number(mult) || 1.5 } });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div><Label>Multiplier</Label><Input type="number" step="0.05" value={mult} onChange={(e) => setMult(e.target.value)} /></div>
      </div>
      <Button onClick={submit} disabled={busy || !code || !name}>{busy ? "Saving…" : "Add rate"}</Button>
      <p className="text-xs text-muted-foreground">
        Manage the full list on the <Link to="/admin/overtime-rates" className="underline">Overtime rates</Link> page.
      </p>
    </div>
  );
}
...
function CurrencyStep({ onSaved }: { onSaved: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Button onClick={submit} disabled={busy}>{busy ? "Saving…" : "Save currency"}</Button>
    </div>
  );
}
