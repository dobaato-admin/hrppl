...
import {
  ...
  listGrievances,
  updateGrievance,
  listGrievanceComments,
  addGrievanceComment,
  listApprovals,
  requestApproval,
  decideApproval,
  recordCaseAttachment,
  listCaseAttachments,
  deleteCaseAttachment,
  getAttachmentDownloadUrl,
  recordGrievanceAttachment,
  listGrievanceAttachments,
...
function DisciplinePage() {
  ...
  if (!canManage)
    return (
      <AppShell title="Discipline & grievances">
        <Card>
  ...
  return (
    <AppShell
      ...
      subtitle="Manage disciplinary cases, approvals and grievances"
    >
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>{cases.length} total · click a row to open</CardDescription>
        </div>
            ...
            {cases.map((c: any) => {
              ...
              return (
                <TableRow key={c.id} className="cursor-pointer" onClick={() => setActive(c)}>
                  ...
                  <TableCell>
                    <Badge className={`text-white ${sevColor(c.severity)}`}>{c.severity}</Badge>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className={overdue ? "text-status-stuck font-medium" : ""}>
                    {c.due_date ?? "—"}
                  </TableCell>
                  <TableCell>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{form?.id ? "Edit case" : "New case"}</DialogTitle>
          </DialogHeader>
          {form && (
    ...
    <div className="space-y-4">
      <div className="rounded-md border p-3 bg-muted/30">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {caseRow.confidential && (
            <Badge variant="outline" className="text-status-stuck border-status-stuck">
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap gap-2">
          {allowed.length === 0 && (
            <span className="text-sm text-muted-foreground">Case is terminal.</span>
          )}
          {allowed.map((s) => (
            <Button
              key={s}
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-3 gap-3 items-end">
          <div>
            ...
            <Select
              value={assigning.assigned_to || "__none"}
              onValueChange={(v) =>
                setAssigning({ ...assigning, assigned_to: v === "__none" ? "" : v })
              }
            >
              <SelectTrigger>
          ...
          <div>
            ...
            <Input
              type="date"
              value={assigning.due_date}
              onChange={(e) => setAssigning({ ...assigning, due_date: e.target.value })}
            />
          </div>
          <div>
            ...
            <Input
              type="date"
              value={assigning.appeal_deadline}
              onChange={(e) => setAssigning({ ...assigning, appeal_deadline: e.target.value })}
            />
          </div>
...
function CaseApprovals({ caseRow }: { caseRow: any }) {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => listFn({ data: { case_id: caseRow.id } }),
  });
  ...
  async function send(e: React.FormEvent) {
    ...
    try {
      ...
      setOpen(false);
      setForm({ approver_id: "", approver_role: "manager", notes: "" });
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-2">
        ...
        {items.length === 0 && (
          <div className="text-sm text-muted-foreground text-center py-2">No approvals yet</div>
        )}
      </CardContent>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <form onSubmit={send} className="space-y-3">
            ...
            <DialogFooter>
              <Button type="submit" disabled={!form.approver_id}>
                Send
              </Button>
            </DialogFooter>
          </form>
...
function CaseAttachments({ caseRow }: { caseRow: any }) {
  ...
  async function upload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: ["case-attachments", caseRow.id] });
      qc.invalidateQueries({ queryKey: ["case-actions", caseRow.id] });
    } catch (e: any) {
  ...
  async function download(p: string) {
    try {
      const r = await urlFn({ data: { storage_path: p } });
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-1">
        ...
        {items.length === 0 && (
          <div className="text-sm text-muted-foreground text-center py-2">No files</div>
        )}
      </CardContent>
    </Card>
  );
    ...
    try {
      await addFn({
        data: {
          ...
          action_date: form.action_date || undefined,
        },
      });
...
function ApprovalsTab() {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => listFn({ data: { pending_for_me: true } }),
  });
  const items = data?.approvals ?? [];
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Approvals waiting for me</CardTitle>
      ...
      <CardContent className="space-y-3">
          ...
          <div key={a.id} className="rounded-md border p-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-sm font-medium">
                Case {a.case?.case_number ?? a.case?.id?.slice(0, 8)}
              </span>
              <Badge variant="secondary" className="capitalize">
                {a.case?.category}
              </Badge>
              {a.case?.employees && (
                <span className="text-sm text-muted-foreground">
                  · {a.case.employees.first_name} {a.case.employees.last_name}
                </span>
              )}
            </div>
            ...
            <div className="mt-2 flex gap-2 justify-end">
              <Button size="sm" variant="outline" onClick={() => act(a.id, "rejected")}>
                <XCircle className="mr-1 h-4 w-4" />
                Reject
              </Button>
              <Button size="sm" onClick={() => act(a.id, "approved")}>
        ...
        {items.length === 0 && (
          <div className="text-sm text-muted-foreground text-center py-6">No approvals pending</div>
        )}
      </CardContent>
    </Card>
  );
...
function GrievancesTab() {
  ...
  async function setStatus(id: string, status: string) {
    try {
      ...
      qc.invalidateQueries({ queryKey: ["grievances-all"] });
  ...
  async function assign(id: string, userId: string) {
    try {
      await updateFn({ data: { id, assigned_to: userId || null } });
      qc.invalidateQueries({ queryKey: ["grievances-all"] });
    } catch (e: any) {
      toast.error(e?.message ?? "Failed");
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>{grievances.length} total</CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
              ...
              <TableRow key={g.id} className="cursor-pointer" onClick={() => setActive(g)}>
                ...
                <TableCell>
                  {g.is_anonymous
                    ? "Anonymous"
                    : g.filer
                      ? `${g.filer.first_name} ${g.filer.last_name}`
                      : "—"}
                </TableCell>
                <TableCell>
                  <Badge className={`text-white ${sevColor(g.severity)}`}>{g.severity}</Badge>
                </TableCell>
                <TableCell onClick={(e) => e.stopPropagation()}>
                  <Select
                    value={g.assigned_to ?? "__none"}
                    onValueChange={(v) => assign(g.id, v === "__none" ? "" : v)}
                  >
...
export function GrievanceThread({
  ...
  const { data } = useQuery({
    ...
    queryFn: () => listFn({ data: { grievance_id: grievance.id } }),
  });
    ...
    <div className="space-y-4">
      <div className="rounded-md border p-3 bg-muted/30">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <Badge className={`text-white ${sevColor(grievance.severity)}`}>
            {grievance.severity}
          </Badge>
          <Badge variant="secondary" className="capitalize">
            {grievance.status}
      ...
      <div className="space-y-2">
        {comments.map((c: any) => (
          <div
            key={c.id}
            className={`rounded-md border p-3 ${c.is_internal ? "border-status-pending bg-status-pending/5" : ""}`}
          >
            ...
            {c.is_internal && (
              <Badge variant="outline" className="mt-1 text-status-pending border-status-pending">
                Internal
              </Badge>
            )}
...
export function GrievanceAttachments({
  ...
  async function upload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      toast.success("Uploaded");
      qc.invalidateQueries({ queryKey: ["grievance-attachments", grievance.id] });
    } catch (e: any) {
      toast.error(e?.message ?? "Upload failed");
    } finally {
      setBusy(false);
