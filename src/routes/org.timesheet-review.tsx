...
import {
  ...
  rejectTimesheet,
} from "@/lib/timesheet-workflow.functions";
...
export const Route = createFileRoute("/org/timesheet-review")({
  head: () => ({ meta: [{ title: "Timesheet review — hrppl" }] }),
  component: () => (
    <AdminGate feature="org.timesheetReview">
      <TimesheetReviewPage />
    </AdminGate>
  ),
});
...
function TimesheetReviewPage() {
  ...
  const submittedCount =
    statusFilter === "submitted"
      ? pending.length
      : pending.filter((t: any) => t.status === "submitted").length;
  ...
  const filteredPending = pending.filter((t: any) => {
    ...
    const name =
      `${t.employees?.first_name ?? ""} ${t.employees?.last_name ?? ""} ${t.employees?.employee_number ?? ""}`.toLowerCase();
  ...
  useEffect(() => {
    setSelected(new Set());
  }, [statusFilter, employeeQuery, pendingData]);
  const submittedVisibleIds = filteredPending
    .filter((t: any) => t.status === "submitted")
    .map((t: any) => t.id);
  const allSelected =
    submittedVisibleIds.length > 0 && submittedVisibleIds.every((id: string) => selected.has(id));
  function toggleAll() {
                  ...
                  <Table>
                    ...
                    <TableBody>
                      ...
                      {!pendingLoading && filteredPending.length === 0 && (
                        <TableRow>
                          <TableCell colSpan={8} className="text-center text-muted-foreground py-6">
                            {employeeQuery ? "No matches for that search." : "Nothing here."}
                          </TableCell>
                        </TableRow>
                      )}
                      {filteredPending.map((t: any) => {
                        const label = t.employees
                        ...
                        return (
                          <TableRow key={t.id} className={selected.has(t.id) ? "bg-muted/40" : ""}>
                            ...
                            <TableCell className="text-right tabular-nums">
                              {Number(t.total_hours).toFixed(2)}
                            </TableCell>
                            ...
                            <TableCell
                              ...
                              title={t.rejection_reason || t.notes || ""}
                            >
                            ...
                            <TableCell className="text-right">
                              {isSubmitted ? (
                                <div className="flex justify-end gap-2">
                                  <Button size="sm" onClick={() => doApprove(t.id)}>
                                    <CheckCircle2 className="h-4 w-4 mr-1" />
                                    Approve
                                  </Button>
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    onClick={() => setRejectFor({ id: t.id, label })}
                                  >
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                      ...
                      {rows.map((r: any) => {
                        ...
                        return (
                          <TableRow
                            ...
                            className={r.flag_suspicious ? "bg-destructive/5" : ""}
                          >
                            <TableCell>{r.work_date}</TableCell>
                            <TableCell>{label}</TableCell>
                            <TableCell className="text-right tabular-nums">
                              {Number(r.rostered_hours).toFixed(2)}
                            </TableCell>
                            <TableCell className="text-right tabular-nums">
                              {Number(r.attendance_hours).toFixed(2)}
                            </TableCell>
                            <TableCell className="text-right tabular-nums">
                              {Number(r.claimed_hours).toFixed(2)}
                            </TableCell>
                            <TableCell
                              className={`text-right tabular-nums ${Math.abs(variance) > 0.5 ? "text-destructive font-medium" : ""}`}
                            >
                              {variance > 0 ? "+" : ""}
                              {variance.toFixed(2)}
                            </TableCell>
                ...
                <p className="text-xs text-muted-foreground">
                  Δ &gt; ±30 min between claimed and attendance is flagged. Rostered hours will
                  populate when the roster module is enabled.
                </p>
              </CardContent>
            </Card>
          </TabsContent>
