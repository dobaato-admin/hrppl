  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // Restrict to privileged roles — basic employees must not trigger sends.
        const { data: roleRows } = await supabase
          .from('user_roles')
          .select('role')
          .eq('user_id', user.id)
        const allowedRoles = new Set(['super_admin', 'org_admin', 'manager', 'regional_admin'])
        const hasPrivilegedRole = (roleRows ?? []).some((r: any) => allowedRoles.has(r.role))
