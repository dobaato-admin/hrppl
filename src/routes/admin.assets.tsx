...
function AssetsPage() {
  ...
  const assignQ = useQuery({
    ...
    queryFn: () => fList({ data: { openOnly: false } }),
  });
  ...
  const confirmM = useMutation({
    mutationFn: (v: { id: string; cond: "good" | "damaged" | "lost"; notes?: string }) =>
      fConfirm({
        data: { assignmentId: v.id, returnCondition: v.cond, confirmationNotes: v.notes },
      }),
    onSuccess: () => {
  ...
  const disputeM = useMutation({
    mutationFn: (v: { id: string; notes: string }) =>
      fConfirm({ data: { assignmentId: v.id, disputed: true, confirmationNotes: v.notes } }),
    onSuccess: () => {
  ...
  const pendingReturns = assignments.filter((a: any) =>
    ["reported_partial", "reported_full"].includes(a.return_status),
  );

  return (
    <AppShell
      title="Asset register"
      subtitle="Track company-owned items and who currently has them. Assign during onboarding or any time, and reclaim on exit."
    >
      <div className="p-4">
        <Tabs defaultValue="register">
          <TabsList>
            ...
            <TabsTrigger value="approvals">
              ...
              {pendingApprovals.length > 0 && (
                <Badge className="ml-1">{pendingApprovals.length}</Badge>
              )}
            </TabsTrigger>
            <TabsTrigger value="returns">
              Returns{" "}
              {pendingReturns.length > 0 && <Badge className="ml-1">{pendingReturns.length}</Badge>}
            </TabsTrigger>
              ...
              <CardContent className="space-y-2">
                {(assetsQ.data?.assets ?? []).map((a: any) => (
                  ...
                  >
                    ...
                    {a.serial_number && (
                      <span className="text-xs text-muted-foreground">SN: {a.serial_number}</span>
                    )}
                  </div>
                ))}
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                {assignments.map((a: any) => {
                  const emp = (empsQ.data?.employees ?? []).find(
                    (e: any) => e.id === a.employee_id,
                  );
                  return (
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {emp && (
                          <Link
                            ...
                            params={{ employeeId: a.employee_id }}
                        ...
                        {a.return_status === "confirmed" && (
                          <Badge>
                            Returned{" "}
                            {a.returned_at ? new Date(a.returned_at).toLocaleDateString() : ""}
                          </Badge>
                        )}
                        {a.return_status === "reported_partial" && (
                          <Badge variant="outline">Partial — awaiting HR</Badge>
                        )}
                        {a.return_status === "reported_full" && (
                          <Badge variant="outline">Reported — awaiting HR</Badge>
                        )}
                        {a.return_status === "disputed" && (
                          <Badge variant="destructive">Disputed</Badge>
                        )}
                        {a.return_status === "pending" &&
                          a.approval_status === "approved" &&
                          a.expected_return_on && (
                            <span className="text-xs text-muted-foreground">
                              Due {a.expected_return_on}
                            </span>
                          )}
                      </div>
                      {(a.quantity_issued > 1 || a.quantity_returned > 0) && (
                        <div className="text-xs text-muted-foreground">
                          Qty issued {a.quantity_issued} · returned {a.quantity_returned}
                        </div>
                      )}
                      {a.condition_notes && (
                        <div className="text-xs text-muted-foreground">
          ...
          <TabsContent value="approvals" className="mt-4">
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Pending assignment approvals</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                {pendingApprovals.length === 0 && (
                    ...
                    >
                      ...
                      <div className="ml-auto flex gap-2">
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => approveM.mutate({ id: a.id, decision: "rejected" })}
                        >
                          Reject
                        </Button>
                        <Button
                          size="sm"
                          onClick={() => approveM.mutate({ id: a.id, decision: "approved" })}
                        >
                          Approve
                        </Button>
                      </div>
          ...
          <TabsContent value="returns" className="mt-4">
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Returns awaiting HR confirmation</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                {pendingReturns.length === 0 && (
                  <p className="text-sm text-muted-foreground">No returns awaiting confirmation.</p>
                )}
                  ...
                  return (
                    <div key={a.id} className="rounded border p-3 text-sm space-y-2">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {emp && (
                          <span>
                            from {emp.first_name} {emp.last_name}
                          </span>
                        )}
                        <Badge variant="outline">
                          {a.return_status === "reported_partial" ? "Partial" : "Full"} —{" "}
                          {a.quantity_returned}/{a.quantity_issued}
                        </Badge>
                        <Badge variant="outline" className="capitalize">
                          {a.return_condition ?? "good"}
                        </Badge>
                      </div>
                      {a.condition_notes && (
                        <p className="text-xs text-muted-foreground">
                          Employee condition note: {a.condition_notes}
                        </p>
                      )}
                      {a.employee_return_notes && (
                        <p className="text-xs text-muted-foreground">
                          Notes: {a.employee_return_notes}
                        </p>
                      )}
                      <div className="flex flex-wrap gap-2">
                        <Button
                          ...
                          onClick={() =>
                            confirmM.mutate({
                              id: a.id,
                              cond: (a.return_condition ?? "good") as any,
                            })
                          }
                        >
                          Confirm as {a.return_condition ?? "good"}
                        </Button>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => confirmM.mutate({ id: a.id, cond: "damaged" })}
                        >
                          Confirm damaged
                        </Button>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => confirmM.mutate({ id: a.id, cond: "lost" })}
                        >
                          Confirm lost
                        </Button>
                        <Button
                          ...
                          variant="destructive"
                          onClick={() => {
                            const n = window.prompt("Dispute reason?");
                            if (n) disputeM.mutate({ id: a.id, notes: n });
                          }}
                        >
                          Dispute
                        </Button>
              ...
              <CardContent className="space-y-3 max-w-xl">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setAssign({ ...assign, assetId: v })}
                  >
                    <SelectTrigger>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label>Context</Label>
                    <Select
                      value={assign.context}
                      onValueChange={(v) => setAssign({ ...assign, context: v })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div>
                    ...
                    <Input
                      type="date"
                      value={assign.expectedReturnOn}
                      onChange={(e) => setAssign({ ...assign, expectedReturnOn: e.target.value })}
                    />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      type="number"
                      min={1}
                      value={assign.quantityIssued}
                      onChange={(e) =>
                        setAssign({ ...assign, quantityIssued: Number(e.target.value || 1) })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      value={assign.conditionNotes}
                      onChange={(e) => setAssign({ ...assign, conditionNotes: e.target.value })}
                      placeholder="e.g. new, minor scratches"
                    />
                  </div>
                </div>
                <div>
                ...
                <Button
                  disabled={!assign.assetId || !assign.employeeId || assignM.isPending}
          ...
          <TabsContent value="add" className="mt-4">
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Add a new asset</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 max-w-xl">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                ...
                <Button
                  disabled={!form.assetTag || !form.name || createM.isPending}
