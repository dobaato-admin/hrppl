...
import {
  ...
  removeAssignment,
  reviewChecklistItem,
  upsertDefaultAssignmentRule,
  deleteDefaultAssignmentRule,
} from "@/lib/onboarding.functions";
import {
  previewOnboardingOverdueEmail,
...
function OrgOnboarding() {
  ...
  async function load() {
    ...
    const [cRes, eRes, pRes, aRes, dRes, depRes, rRes] = await Promise.all([
      ...
      supabase
        .from("employees")
        .select("id,first_name,last_name,employee_number,hire_date,department_id,job_title")
        .eq("tenant_id", tenantId)
        .eq("status", "active"),
      supabase
        .from("onboarding_progress")
        .select("id,employee_id,checklist_id,item_key,approval_status,approval_notes,completed_at")
        .eq("tenant_id", tenantId),
      supabase
      ...
      supabase
        ...
        .select("id,employee_id,file_name,doc_type,visibility,created_at")
        .eq("tenant_id", tenantId)
        .order("created_at", { ascending: false }),
      supabase.from("departments").select("id,name").eq("tenant_id", tenantId).order("name"),
      supabase
        .from("onboarding_default_assignments")
        .select("*")
        .eq("tenant_id", tenantId)
        .order("created_at", { ascending: false }),
    ]);
  ...
  function startEdit(cl: Checklist) {
    ...
    setDraftItems(cl.items.map((i) => ({ ...i })));
    setEditOpen(true);
  }
  ...
  async function doSignOff() {
    ...
    try {
      await fnSignOff({
        data: { assignmentId: detailAssignment.id, notes: signOffNotes || undefined },
      });
  ...
  function assignmentStats(a: Assignment): { done: number; total: number; pct: number } {
    ...
    const keys = new Set(
      (progressByAssignment[`${a.employee_id}:${a.checklist_id}`] ?? []).map((p) => p.item_key),
    );
    ...
    () =>
      detailAssignment
        ? (progressByAssignment[
            `${detailAssignment.employee_id}:${detailAssignment.checklist_id}`
          ] ?? [])
        : [],
  ...
  return (
    <main className="min-h-screen bg-background">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Assignments, checklist templates, document review.
            </p>
          </div>
          <Link to="/org">
      ...
      <section className="mx-auto max-w-6xl px-6 py-8 space-y-6">
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="pay-setup">
            {tenantId ? (
              <PaySetupPanel tenantId={tenantId} tenantCurrency={tenantCurrency} />
            ) : null}
          </TabsContent>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Default checklists are auto-assigned when an employee is created.
                  </CardDescription>
                </div>
                <Button size="sm" onClick={() => startAssign()}>
                  Assign checklist
                </Button>
              </CardHeader>
              <CardContent>
                        ...
                        <TableRow key={a.id}>
                          <TableCell>
                            <div className="font-medium">
                              {e ? `${e.first_name} ${e.last_name}` : a.employee_id.slice(0, 8)}
                            </div>
                    ...
                    {emps.map((e) => {
                      ...
                      return (
                        <TableRow key={e.id}>
                          ...
                          <TableCell className="text-right">
                            <Button size="sm" variant="outline" onClick={() => startAssign(e.id)}>
                              Assign…
                            </Button>
                          </TableCell>
                        </TableRow>
                  ...
                  const groupKeys = stages.length
                    ? [
                        ...
                        ...Object.keys(grouped).filter(
                          (k) => k === "_unstaged" || !stages.some((s) => s.key === k),
                        ),
                      ]
                    : Object.keys(grouped);
                  return (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {cl.items.length} items · {stages.length} stages
                        </div>
                        <div className="mt-2 space-y-2">
                          {groupKeys.map((gk) => {
                            ...
                            const label =
                              gk === "_unstaged"
                                ? "Unassigned"
                                : (stages.find((s) => s.key === gk)?.label ?? gk);
                            return (
                              <div key={gk}>
                      ...
                      {isAdmin && (
                        <Button size="sm" variant="outline" onClick={() => startEdit(cl)}>
                          Edit
                        </Button>
                      )}
                    </div>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Auto-assign checklists to new hires by department or job title. Leave both blank
                    to apply to all new employees.
                  </CardDescription>
                </div>
                {isAdmin && (
                  <Button size="sm" onClick={startNewRule} disabled={!checklists.length}>
                    New rule
                  </Button>
                )}
              </CardHeader>
                ...
                <Table>
                  ...
                  <TableBody>
                      ...
                      return (
                        <TableRow key={r.id}>
                          <TableCell className="font-medium">
                            {cl?.name ?? r.checklist_id.slice(0, 8)}
                          </TableCell>
                          <TableCell>
                            {dep?.name ?? <span className="text-muted-foreground">Any</span>}
                          </TableCell>
                          <TableCell>
                            {r.job_title ?? <span className="text-muted-foreground">Any</span>}
                          </TableCell>
                          <TableCell className="text-sm">
                            {r.due_offset_days} days after hire
                          </TableCell>
                          <TableCell>
                            {r.is_active ? (
                              <Badge>Active</Badge>
                    ...
                    {rules.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={6} className="text-center text-muted-foreground">
                          No rules configured. New hires will use checklists marked "Default for new
                          hires".
                        </TableCell>
                      </TableRow>
                    )}
          ...
          <TabsContent value="documents">
            <Card>
              <CardHeader>
                <CardTitle className="text-base">All employee documents</CardTitle>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {docs.map((d) => {
                      ...
                      return (
                        <TableRow key={d.id}>
                          <TableCell>
                            {e ? `${e.first_name} ${e.last_name}` : d.employee_id}
                          </TableCell>
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Label>Default for new hires (legacy fallback)</Label>
            </div>

            <div>
              <Label>Stages (optional)</Label>
              <p className="text-xs text-muted-foreground mb-1">
                Group items into phases like pre-boarding, day 1, week 1, first month.
              </p>
                ...
                {draftStages.map((s, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => setDraftStages(draftStages.filter((_, i) => i !== idx))}
                    >
                      ×
                    </Button>
                  </div>
              ...
              <div className="space-y-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    setDraftItems([
                      ...draftItems,
                      { key: `step${draftItems.length + 1}`, label: "", required: true },
                    ])
                  }
                >
                  Add item
          ...
          <DialogFooter>
            <Button onClick={saveChecklist} disabled={busy || !draftName.trim()}>
              Save
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={ruleOpen} onOpenChange={setRuleOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{ruleEditing ? "Edit rule" : "New default-assignment rule"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
            ...
            <div>
              ...
              <Input
                value={ruleJobTitle}
                onChange={(e) => setRuleJobTitle(e.target.value)}
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              <div className="text-sm text-muted-foreground">
                ...
                {detailAssignment.signed_off_at && (
                  <span>
                    {" "}
                    · Signed off {new Date(detailAssignment.signed_off_at).toLocaleDateString()}
                  </span>
                )}
              </div>
                ...
                {detailChecklist.items.map((it) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <Badge variant={pr ? "secondary" : "outline"} className="text-xs">
                            {pr ? "Done" : "Pending"}
                          </Badge>
          ...
          <DialogFooter className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={doRemove} disabled={busy}>
              Remove
            </Button>
            <div className="flex-1" />
            <Button variant="outline" onClick={() => doSetStatus("cancelled")} disabled={busy}>
              Cancel assignment
            </Button>
            <Button variant="outline" onClick={() => doSetStatus("in_progress")} disabled={busy}>
              Reopen
            </Button>
...
function OverviewPanel({
  ...
  const overdue = active.filter(
    (a) => a.due_date && a.due_date < today && a.status !== "signed_off",
  );
  const monthStart = new Date();
  monthStart.setDate(1);
  monthStart.setHours(0, 0, 0, 0);
  const signedOffMonth = assignments.filter(
    (a) => a.status === "signed_off" && a.signed_off_at && new Date(a.signed_off_at) >= monthStart,
  );
  const avgPct =
    active.length === 0
      ? 0
      : Math.round(active.reduce((s, a) => s + assignmentStats(a).pct, 0) / active.length);
  ...
  const newHiresMissing = emps.filter(
    (e) => e.hire_date >= cutoffStr && !empsWithAssignments.has(e.id),
  );
        ...
        <CardContent>
          {newHiresMissing.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              All recent hires have onboarding assignments.
            </p>
          ) : (
            <Table>
              <TableHeader>
...
function ReminderPreviewPanel({
  ...
  const matchedAssignment = useMemo(() => {
    ...
    return (
      assignments.find((a) => a.employee_id === empId && a.checklist_id === checklistId) ?? null
    );
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        {matchedAssignment && (
          <div className="text-xs text-muted-foreground">
            Using existing assignment due {matchedAssignment.due_date ?? "—"} (status{" "}
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button
            variant="outline"
            onClick={loadPreview}
            disabled={loading || !empId || !checklistId}
          >
