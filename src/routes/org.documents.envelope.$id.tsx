        ...
        <div className="flex items-center justify-between">
          ...
          {data?.envelope && (
            <div className="flex items-center gap-2">
              {!["completed", "cancelled", "declined", "expired"].includes(
                data.envelope.status,
              ) && (
                <Button size="sm" variant="outline" disabled={busy} onClick={doRemind}>
                  <BellRing className="h-3.5 w-3.5 mr-1" />
                  {data.envelope.reminder_count
                    ? `Send reminder (${data.envelope.reminder_count} sent)`
                    : "Send reminder"}
                </Button>
              )}
              {data.envelope.status === "completed" && data.envelope.certificate_token && (
                <a
                  href={`/sign/certificate/${data.envelope.certificate_token}`}
                  target="_blank"
                  rel="noreferrer"
                >
                  <Button size="sm">
                    <FileCheck2 className="h-3.5 w-3.5 mr-1" /> View signed copy
                  </Button>
                </a>
              )}
            </div>
              ...
              <CardHeader>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <CardDescription>
                      To {data.envelope.recipient_name} ({data.envelope.recipient_email}) ·{" "}
                      {data.envelope.doc_type.replace(/_/g, " ")}
                      {data.envelope.last_reminder_at && (
                        <>
                          {" "}
                          · Last reminder{" "}
                          {new Date(data.envelope.last_reminder_at).toLocaleString()}
                        </>
                      )}
                    </CardDescription>
                  </div>
              ...
              <CardContent>
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: sanitizeDocHtml(data.envelope.body_html_snapshot),
                  }}
                />
              </CardContent>
            </Card>
                    ...
                    data.events.map((e: any) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          {e.metadata?.audit_hash && (
                            <span className="font-mono text-[10px] text-muted-foreground">
                              {e.metadata.audit_hash.slice(0, 16)}…
                            </span>
                          )}
                        </div>
