...
function SecurityFindingsPage() {
  ...
  const [form, setForm] = useState({ status: "open", ticket_url: "", fixed_in_commit: "", remediation: "" });
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (search && !`${r.title} ${r.internal_id}`.toLowerCase().includes(search.toLowerCase())) return false;
      return true;
    });
  }, [rows, statusFilter, scannerFilter, search]);

  const counts = useMemo(() => {
    const c = { total: rows.length, open: 0, fixed: 0, ignored: 0, accepted_risk: 0, error_open: 0 };
    for (const r of rows) {
      (c as any)[r.status]++;
    ...
    <AppShell>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            All scan results over time across Wiz, Supabase linter, dependency audit, CodeQL and connectors.
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="flex flex-col md:flex-row md:items-center gap-3">
          <Input placeholder="Search title or id…" value={search} onChange={(e) => setSearch(e.target.value)} className="max-w-xs" />
            ...
            <TableBody>
              ...
              {filtered.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>
                    {r.ticket_url ? (
                      <a href={r.ticket_url} target="_blank" rel="noreferrer" className="text-primary inline-flex items-center gap-1 text-xs">
                        ticket <ExternalLink className="size-3" />
                      </a>
                    ) : <span className="text-xs text-muted-foreground">—</span>}
                  </TableCell>
                  <TableCell className="text-xs">
                    {r.status === "fixed" || r.status === "ignored" || r.status === "accepted_risk" ? (
                      <div>
                        <div>{r.resolved_by_name ?? "—"}</div>
                        {r.resolved_at && <div className="text-muted-foreground">{new Date(r.resolved_at).toLocaleDateString()}</div>}
                        {r.fixed_in_commit && (
                          <a href={r.fixed_in_commit.startsWith("http") ? r.fixed_in_commit : `#${r.fixed_in_commit}`}
                             target="_blank" rel="noreferrer"
                             className="text-primary font-mono">{r.fixed_in_commit.slice(0, 10)}</a>
                        )}
                      </div>
          ...
          {editing && (
            <div className="space-y-3">
              ...
              <div>
                ...
                <Input value={form.fixed_in_commit} onChange={(e) => setForm((f) => ({ ...f, fixed_in_commit: e.target.value }))} placeholder="commit SHA or PR URL" />
              </div>
              <div>
                <label className="text-xs font-medium">Remediation notes</label>
                <Textarea value={form.remediation} onChange={(e) => setForm((f) => ({ ...f, remediation: e.target.value }))} rows={4} />
              </div>
            </div>
          )}
          <DialogFooter>
