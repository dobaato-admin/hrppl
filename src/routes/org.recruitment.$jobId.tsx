...
function JobBoard() {
  ...
  async function refresh() {
    const r = await get({ data: { id: jobId } });
    setData(r);
  }
  useEffect(() => {
    refresh();
  ...
  async function onDrop(stageId: string) {
    ...
    try {
      ...
      const status =
        stage?.kind === "hired" ? "hired" : stage?.kind === "rejected" ? "rejected" : "active";
  ...
  async function moveStage(idx: number, dir: -1 | 1) {
    ...
    try {
      await reorder({ data: { job_id: jobId, order } });
      await refresh();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {data.job.location ?? "—"} • {data.candidates.length} candidates
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button asChild variant="outline" size="sm">
            <Link to="/org/recruitment">All roles</Link>
          </Button>
        </div>
      </div>
      ...
      <div
        ...
        style={{ gridTemplateColumns: `repeat(${data.stages.length}, minmax(220px, 1fr))` }}
      >
            ...
            >
              ...
              <div className="space-y-2">
                {inStage.map((c: any) => (
                  <Link
                    key={c.id}
                    to="/org/recruitment/candidate/$candidateId"
