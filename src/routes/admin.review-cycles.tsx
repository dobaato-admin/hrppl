...
import {
  ...
  setCycleStatus,
  deleteCycle,
  getKpiWeightSettings,
  updateKpiWeightSettings,
...
function Page() {
  ...
  const canAccess = can("org.reviewCycles", roles);
  ...
  async function create() {
    if (!label || !starts || !ends) {
      toast.error("Label, start, end are required");
      return;
    }
    try {
      await upsertFn({
        data: { label, starts_on: starts, ends_on: ends, reminder_days_before: reminderDays },
      });
  ...
  async function remove(id: string) {
    if (
      !confirm("Delete this cycle? Submitted scores remain but employees cannot submit against it.")
    )
      return;
    try {
      await delFn({ data: { id } });
  ...
  async function saveSettings() {
    ...
    if (Number.isNaN(t) || t < 0 || t > 25) {
      toast.error("Tolerance must be 0–25");
      return;
    }
    try {
  ...
  return (
    <AppShell
      title="KPI review cycles"
      subtitle="Open a cycle to allow submissions, close it to lock scoring. Employees are notified automatically."
    >
      <section className="mx-auto max-w-5xl space-y-6 p-4 md:p-6">
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
              ...
              <CardTitle>KPI weight rules</CardTitle>
            </div>
            <CardDescription>
              Block duty saves unless weights sum to 100% (within the allowed tolerance).
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-5 gap-3">
            ...
            <div className="space-y-1">
              <Label>Remind (days before close)</Label>
              <Input
                type="number"
                min={0}
                max={60}
            ...
            {cycles.map((c: any) => (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    ...
                    {c.last_reminder_sent_at &&
                      ` · last reminder ${new Date(c.last_reminder_sent_at).toLocaleDateString()}`}
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  {c.status !== "open" && (
                    <Button size="sm" variant="outline" onClick={() => setStatus(c.id, "open")}>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Track who has completed their self-review and manager review for a cycle.
            </CardDescription>
          </CardHeader>
                  ...
                  <tbody>
                    {(statusQ.data?.rows ?? []).length === 0 && (
                      <tr>
                        <td colSpan={4} className="p-3 text-center text-muted-foreground">
                          No employees with active duties.
                        </td>
                      </tr>
                    )}
                    {(statusQ.data?.rows ?? []).map((r: any) => (
                      <tr key={r.employee_id} className="border-t">
                        ...
                        <td className="p-2">
                          {statusBadge(r.self_status)}{" "}
                          <span className="text-xs text-muted-foreground ml-2">
                            {r.self_submitted}/{r.total_duties}
                          </span>
                        </td>
                        <td className="p-2">
                          {statusBadge(r.reviewer_status)}{" "}
                          <span className="text-xs text-muted-foreground ml-2">
                            {r.reviewer_submitted}/{r.total_duties}
                          </span>
                        </td>
                      </tr>
                    ))}
