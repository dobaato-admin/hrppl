...
import { listDesignations, upsertDesignation, deleteDesignation, seedDesignationPreset } from "@/lib/hr-extras.functions";
...
function DesignationsPage() {
  ...
  const [form, setForm] = useState<any>({ title: "", grade: "", currency_code: "USD", is_active: true });
  ...
  function startNew() { setForm({ title: "", grade: "", currency_code: "USD", is_active: true }); setOpen(true); }
  function startEdit(d: any) { setForm({ ...d }); setOpen(true); }
  ...
  async function remove(id: string) {
    ...
    try { await delFn({ data: { id } }); toast.success("Deleted"); qc.invalidateQueries({ queryKey: ["designations"] }); }
    catch (e: any) { toast.error(e?.message ?? "Failed"); }
  }
  ...
  async function applyPreset() {
    ...
    setBusy(true);
    try {
      const res = await seedFn({ data: {
        currency_code: presetCurrency.toUpperCase(),
        department_id: presetDept || null,
        designations: preset.designations,
      }});
      toast.success(`Added ${res.inserted} designation${res.inserted === 1 ? "" : "s"}${res.skipped ? ` (${res.skipped} skipped, already exist)` : ""}`);
      setPresetOpen(false);
      qc.invalidateQueries({ queryKey: ["designations"] });
  ...
  return (
      ...
      actions={
        <div className="flex flex-wrap items-center gap-2">
          <Dialog open={presetOpen} onOpenChange={setPresetOpen}>
            ...
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                ...
                <DialogDescription>
                  Pick an industry template — we'll create the ladder for you. Titles that already exist will be skipped.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-3">
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input maxLength={3} value={presetCurrency} onChange={(e) => setPresetCurrency(e.target.value.toUpperCase())} />
                  </div>
                  <div className="space-y-2">
                    <Label>Department (optional)</Label>
                    <Select value={presetDept || "__none__"} onValueChange={(v) => setPresetDept(v === "__none__" ? "" : v)}>
                ...
                {activePreset && (
                  <div className="rounded-md border bg-muted/30 p-3">
                    ...
                    <div className="mt-2 max-h-56 overflow-y-auto">
                      <Table>
                        <TableHeader><TableRow><TableHead>Grade</TableHead><TableHead>Title</TableHead><TableHead className="text-right">Band</TableHead></TableRow></TableHeader>
                        <TableBody>
                          {activePreset.designations.map((d) => (
                            <TableRow key={d.title}>
                              ...
                              <TableCell className="text-right text-xs text-muted-foreground">{d.min_salary?.toLocaleString()} – {d.max_salary?.toLocaleString()}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild><Button size="sm" onClick={startNew}><Plus className="mr-1 h-4 w-4" /> New designation</Button></DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>{form.id ? "Edit" : "New"} designation</DialogTitle></DialogHeader>
            <form onSubmit={save} className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-2"><Label>Min salary</Label><Input type="number" value={form.min_salary ?? ""} onChange={(e) => setForm({ ...form, min_salary: e.target.value })} /></div>
                <div className="space-y-2"><Label>Max salary</Label><Input type="number" value={form.max_salary ?? ""} onChange={(e) => setForm({ ...form, max_salary: e.target.value })} /></div>
                <div className="space-y-2"><Label>Currency</Label><Input maxLength={3} value={form.currency_code ?? ""} onChange={(e) => setForm({ ...form, currency_code: e.target.value.toUpperCase() })} /></div>
              </div>
    ...
    >
      <section className="mx-auto max-w-6xl space-y-6 p-4 md:p-6">
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2"><BadgeCheck className="h-5 w-5 text-primary" /><CardTitle>Catalog</CardTitle></div>
            <CardDescription>Used by promotions, pay rates, and recruitment offers.</CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? <div className="text-sm text-muted-foreground">Loading…</div> : (
              <Table>
                <TableHeader><TableRow><TableHead>Title</TableHead><TableHead>Grade</TableHead><TableHead>Department</TableHead><TableHead>Salary band</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
                <TableBody>
                  {(data?.designations ?? []).length === 0 ? (
                    <TableRow><TableCell colSpan={6} className="text-muted-foreground">No designations yet.</TableCell></TableRow>
                  ) : data!.designations.map((d: any) => (
                    <TableRow key={d.id}>
                      <TableCell className="font-medium">{d.title}{d.code && <div className="text-xs text-muted-foreground">{d.code}</div>}</TableCell>
                      <TableCell>{d.grade ?? "—"}</TableCell>
                      <TableCell>{d.departments?.name ?? "—"}</TableCell>
                      <TableCell className="text-xs">{d.min_salary || d.max_salary ? `${d.currency_code ?? ""} ${d.min_salary ?? "—"} – ${d.max_salary ?? "—"}` : "—"}</TableCell>
