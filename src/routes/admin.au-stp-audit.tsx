...
function downloadGapsCsv(gaps: any[], missingSuper: any[]) {
  const rows = [
    ...
    ...gaps.flatMap((g) =>
      g.missing.map((m: string) => [g.employee_number ?? "", g.name, "STP2", m]),
    ),
  ...
  a.href = url;
  a.download = `au-stp-audit-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
...
function Page() {
  ...
  if (!a.ok)
    return (
      <AppShell title="AU STP2 audit">
        <div className="p-4 text-sm text-muted-foreground">{a.reason}</div>
      </AppShell>
    );
    ...
    <AppShell title="AU STP Phase 2 & Payday Super audit" subtitle={`Tenant: ${a.tenant.name}`}>
      <div className="p-4 space-y-4">
          ...
          <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm flex items-start gap-2">
            ...
            <div>
              ...
              <div className="text-muted-foreground">
                Resolve the employer and employee gaps below before building or submitting a pay
                event.
              </div>
            </div>
          </div>
          ...
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-sm">
                Employee readiness ({a.employees.with_gaps} of {a.employees.total} have gaps)
              </CardTitle>
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => downloadGapsCsv(a.employees.gaps, a.employees.missing_super_choice)}
            >
              ...
              <TableBody>
                ...
                {a.employees.gaps.map((e: any) => (
                  <TableRow key={e.id}>
                    ...
                    <TableCell>
                      <div className="text-xs">{e.missing.join(" · ")}</div>
                    </TableCell>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">
              Super fund choice ({a.employees.missing_super_count} missing)
            </CardTitle>
            <CardDescription>
              Employees without a recorded super choice; falls back to default fund / stapled
              lookup.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-0">
                ...
                {a.employees.missing_super_choice.length === 0 && (
                  <TableRow>
                    <TableCell
                      ...
                      className="text-center text-sm text-muted-foreground py-6"
                    >
                      All employees have a super choice.
                    </TableCell>
                  </TableRow>
                )}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">
              Payday Super (effective 1 July 2026 — 7-day rule)
            </CardTitle>
            <CardDescription>
              SG contributions must reach the fund within 7 calendar days of payday.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span>Payday Super enabled</span>
              <StatusPill ok={a.paydaySuper.enabled} />
            </div>
            <div className="flex justify-between">
              <span>7-day rule achievable with current config</span>
              <StatusPill ok={a.paydaySuper.sevenDayRuleAchievable} />
            </div>
            <div className="flex justify-between">
              <span>Overdue obligations</span>
              <Badge variant={a.paydaySuper.overdue > 0 ? "destructive" : "outline"}>
                {a.paydaySuper.overdue}
              </Badge>
            </div>
                  ...
                  <TableBody>
                    {a.paydaySuper.recent.slice(0, 8).map((o: any) => (
                      <TableRow key={o.id}>
                        ...
                        <TableCell>
                          <Badge variant={o.status === "paid" ? "outline" : "destructive"}>
                            {o.status}
                          </Badge>
                        </TableCell>
