...
import {
  ...
  runReconciliation,
} from "@/lib/geofence-reconciliation.functions";
...
export const Route = createFileRoute("/admin/geofences")({
  ...
  component: () => (
    <AdminGate feature="org.geofences">
      <GeofencePage />
    </AdminGate>
  ),
});
    ...
    >
        ...
        <Tabs defaultValue="fences">
          <TabsList>
            ...
            <TabsTrigger value="simulator">
              <FlaskConical className="h-3.5 w-3.5 mr-1" /> Simulator
            </TabsTrigger>
            <TabsTrigger value="reconciliation">
              <ClipboardList className="h-3.5 w-3.5 mr-1" /> Reconciliation
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="fences" className="pt-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  When an envelope requires geofencing, signers must be within one of these circles.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Min accuracy</TableHead>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing.id ? "Edit geofence" : "New geofence"}</DialogTitle>
          </DialogHeader>
              ...
              return (
                <Tabs defaultValue={HAS_GMAPS_KEY ? "map" : "device"}>
                  <TabsList>
                    ...
                    <TabsTrigger value="map" disabled={!HAS_GMAPS_KEY}>
                      Map picker
                    </TabsTrigger>
                  </TabsList>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditing({ ...editing, min_accuracy_meters: Number(e.target.value) || 100 })
                  }
                />
                <p className="text-[11px] text-muted-foreground mt-1">
                  Captures worse than this are flagged in the audit log.
                </p>
              </div>
              <div className="flex items-end gap-3">
                <div className="flex items-center gap-2 flex-1">
                  <Switch
                    ...
                    onCheckedChange={(v) =>
                      setEditing({ ...editing, background_tracking_enabled: !!v })
                    }
                  />
                  <div className="flex-1">
                    <Label className="text-xs">Background tracking</Label>
                    <p className="text-[11px] text-muted-foreground">
                      Keeps watching while the tab/PWA stays open.
                    </p>
                  </div>
...
function AuditPanel({ geofences }: { geofences: any[] }) {
  ...
  function exportCsv() {
    ...
    const lines = [headers.join(",")].concat(
      rows.map((r) => headers.map((h) => csvCell((r as any)[h])).join(",")),
    );
  ...
  function exportPdf() {
    ...
    const html = `<!doctype html><html><head><title>Geofence audit</title><style>${css}</style></head><body>
      ...
      <tbody>${rows
        .map(
          (r) => `<tr class="${r.is_suspicious ? "sus" : ""}">
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Every capture, manual edit, permission event, and suspicious flag.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="flex flex-wrap items-end gap-2">
          ...
          <label className="flex items-center gap-2 text-xs">
            <Switch checked={suspiciousOnly} onCheckedChange={(v) => setSuspiciousOnly(!!v)} />{" "}
            Suspicious only
          </label>
...
function SimulatorPanel({ geofences }: { geofences: any[] }) {
  ...
  async function saveTrace() {
    try {
      ...
      await save({
        data: { name: draft.name || "Untitled trace", description: draft.description, points },
      });
      toast.success("Trace saved");
      setOpen(false);
      setDraft({ name: "", description: "", pointsText: "" });
      refresh();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  async function runSim() {
    ...
    audit({
      data: {
        ...
        source: "simulation",
        metadata: { trace_id: trace.id, points: trace.points.length },
      },
    }).catch(() => {});
    for (let i = 0; i < trace.points.length; i++) {
      ...
      setLog((l) => [
        ...l,
        `#${i + 1} d=${Math.round(dist)}m acc=±${p.accuracy_m ?? "?"}m ${event}${flagged ? " ⚠ low-acc" : ""}`,
      ]);
      audit({
        data: {
          geofence_id: fence.id,
    ...
    <div className="grid md:grid-cols-2 gap-4">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Replay these against a fence — your real GPS is untouched.
            </CardDescription>
          </div>
          <Button size="sm" onClick={() => setOpen(true)}>
            <Plus className="h-3.5 w-3.5 mr-1" /> New trace
          </Button>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              ...
              {traces.map((t) => (
                <TableRow key={t.id} className={selectedTrace === t.id ? "bg-muted/50" : ""}>
                  <TableCell className="cursor-pointer" onClick={() => setSelectedTrace(t.id)}>
                    {t.name}
                  </TableCell>
                  <TableCell>{t.points?.length ?? 0}</TableCell>
                  <TableCell className="text-right">
                    <Button size="sm" variant="ghost" onClick={() => setSelectedTrace(t.id)}>
                      Pick
                    </Button>
                    <Button
                      size="sm"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Drive enter/exit logic against any fence, with audit entries.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Select value={fenceId} onValueChange={setFenceId}>
                <SelectTrigger className="h-9">
                  <SelectValue placeholder="Pick fence" />
                </SelectTrigger>
                <SelectContent>
                  {geofences.map((g) => (
                    <SelectItem key={g.id} value={g.id}>
            ...
            <div>
              ...
              <Select value={selectedTrace} onValueChange={setSelectedTrace}>
                <SelectTrigger className="h-9">
                  <SelectValue placeholder="Pick trace" />
                </SelectTrigger>
                <SelectContent>
                  {traces.map((t) => (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>New simulation trace</DialogTitle>
          </DialogHeader>
          <div className="space-y-2">
            ...
            <div>
              <Label>
                Points (one per line: <code>lat, lng, accuracy_m</code>)
              </Label>
              <Textarea
                rows={8}
                ...
                onChange={(e) => setDraft({ ...draft, pointsText: e.target.value })}
              />
            </div>
          </div>
          <DialogFooter>
            <Button onClick={saveTrace}>Save trace</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function ReconciliationPanel({ geofences }: { geofences: any[] }) {
  ...
  const [rows, setRows] = useState<any[]>([]);
  const [status, setStatus] = useState<"open" | "reviewed" | "resolved" | "dismissed" | "all">(
    "open",
  );
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            Mismatches between geofence captures and attendance punches.
          </CardDescription>
        </div>
            ...
            {rows.map((r) => (
              <TableRow key={r.id}>
                ...
                <TableCell className="text-right space-x-1">
                  {r.status === "open" && (
                    <>
                      <Button size="sm" variant="ghost" onClick={() => act(r.id, "reviewed")}>
                        Review
                      </Button>
                      <Button size="sm" variant="ghost" onClick={() => act(r.id, "resolved")}>
                        Resolve
                      </Button>
