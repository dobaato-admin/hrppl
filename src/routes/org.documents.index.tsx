...
function EnvelopesPage() {
  ...
  useEffect(() => {
    loadEmployees(); /* eslint-disable-next-line */
  }, [tenantId]);
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-3">
          <div>
            ...
            <CardDescription>
              Documents sent to employees, contractors, or candidates.
            </CardDescription>
          </div>
              ...
              {rows.length === 0 ? (
                ...
                rows.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell>
                      <Badge variant="outline">{r.doc_type.replace(/_/g, " ")}</Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <span
                        className={`inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium ${STATUS_TONE[r.status] ?? ""}`}
                      >
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-md border p-3 space-y-2">
          <label className="flex items-center gap-2 text-sm font-medium">
            <Checkbox
              checked={useCountersigner}
              onCheckedChange={(c) => setUseCountersigner(!!c)}
            />
          ...
          {useCountersigner && (
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="space-y-1">
                ...
                <Input
                  value={countersignerRole}
                  onChange={(e) => setCountersignerRole(e.target.value)}
        ...
        <div className="space-y-2">
          ...
          <div className="max-h-64 overflow-y-auto rounded-md border">
            {filtered.map((e) => (
              <label
                key={e.id}
                className="flex items-center gap-2 border-b px-2 py-1.5 last:border-b-0 hover:bg-accent"
              >
                <Checkbox
      ...
      <DialogFooter>
        <Button
          disabled={
            ...
            !subject ||
            (!templateId && !bodyHtml) ||
            (useCountersigner && !countersignerEmpId)
          }
          onClick={() =>
            props.onSubmit({
