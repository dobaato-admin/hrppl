...
function MyExpensesPage() {
  ...
  const claimTotal = lines.reduce((sum, l) => sum + (Number(l.amount) || 0), 0);
  const previewRoutingFn = useServerFn(previewClaimRouting);
  const { data: routing } = useQuery({
    queryKey: ["claim-routing", claimTotal],
    queryFn: () => previewRoutingFn({ data: { amount: claimTotal } }),
    enabled: open && claimTotal > 0,
    staleTime: 60_000,
  });
  useEffect(() => {
    refresh();
  ...
  async function uploadReceipt(file: File, idx: number) {
    ...
    if (file.size > RECEIPT_MAX_BYTES) {
      return toast.error(
        `Receipt too large (${(file.size / 1024 / 1024).toFixed(1)}MB) — max 10MB.`,
      );
    }
  ...
  async function submit(submitAfter: boolean) {
    ...
    if (lines.some((l) => !l.amount || l.amount <= 0))
      return toast.error("All lines need an amount");
    setWorking(true);
    try {
      await save({
        data: {
          ...
          lines: lines.map((l) => ({
            ...
            mileage_km: l.mileage_km ? Number(l.mileage_km) : undefined,
            tax_amount: l.tax_amount ? Number(l.tax_amount) : undefined,
          })),
          submit: submitAfter,
        },
      });
  ...
  return (
    <AppShell
      title="My expenses"
      subtitle="Submit receipts and track reimbursements"
      actions={
      ...
      <div className="p-4 md:p-6 space-y-4">
          ...
          <Card>
            <CardHeader className="flex flex-row items-start justify-between gap-2">
              <div>
                ...
                <CardDescription>
                  Approved, paid and outstanding totals across all your claims.
                </CardDescription>
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Drafts can be edited before submission. Submitted claims are reviewed by your manager
              or finance team.
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                {claims.length === 0 ? (
                  ...
                  claims.map((c) => (
                    <TableRow key={c.id}>
                      ...
                      <TableCell>
                        <Badge className={STATUS_TONE[c.status] + " border-0 capitalize"}>
                          {c.status}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-right font-mono">
                        {c.currency} {Number(c.total_amount).toFixed(2)}
                      </TableCell>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                ...
                <Input
                  ...
                  placeholder="Sept client travel"
                />
              </div>
              <div>
                <Label>Currency</Label>
                <Input
                  value={lines[0]?.currency ?? "AUD"}
                  onChange={(e) =>
                    setLines((ls) =>
                      ls.map((l) => ({ ...l, currency: e.target.value.toUpperCase() })),
                    )
                  }
                />
              </div>
            ...
            <div className="space-y-2">
              ...
              {catState === "empty" && (
                <p className="rounded-md border border-dashed p-3 text-xs text-muted-foreground">
                  ...
                  <Link to="/org/expenses" className="underline">
                    Expenses &rarr; Categories
                  </Link>{" "}
              ...
              {lines.map((l, i) => (
                <div key={i} className="grid gap-2 rounded-md border p-3 md:grid-cols-12">
                  <Input
                    ...
                    onChange={(e) =>
                      setLines((ls) =>
                        ls.map((x, j) => (j === i ? { ...x, expense_date: e.target.value } : x)),
                      )
                    }
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setLines((ls) =>
                        ls.map((x, j) => (j === i ? { ...x, category_id: v || null } : x)),
                      )
                    }
                  >
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setLines((ls) =>
                        ls.map((x, j) => (j === i ? { ...x, merchant: e.target.value } : x)),
                      )
                    }
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setLines((ls) =>
                        ls.map((x, j) => (j === i ? { ...x, amount: Number(e.target.value) } : x)),
                      )
                    }
                  ...
                  <div className="md:col-span-2 flex items-center gap-1">
                    {l.receipt_path ? (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => openReceipt(l.receipt_path!)}
                      >
                      ...
                      <label className="inline-flex cursor-pointer items-center gap-1 rounded-md border px-2 py-1 text-xs hover:bg-muted">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            e.target.files?.[0] && uploadReceipt(e.target.files[0], i)
                          }
                        />
                      </label>
                    )}
            ...
            <div className="flex justify-end font-mono text-sm">
              Total: {lines[0]?.currency ?? "AUD"} {claimTotal.toFixed(2)}
            </div>
            {/* W5 P3 · previewClaimRouting had no caller, so the approval rules
                configured on /admin/expenses were invisible until after
                submitting — the one moment you cannot change the claim. Routing
                depends on the amount, so this re-reads as the total changes. */}
            {routing && routing.approvers.length > 0 && (
              <div className="rounded-md border bg-muted/40 p-2.5 text-xs">
                <span className="text-muted-foreground">This claim will go to </span>
                <span className="font-medium">
                  {routing.approvers
                    .map((a: any) => a.full_name ?? a.email ?? "an approver")
                    .join(" → ")}
                </span>
              </div>
            )}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => submit(false)} disabled={working}>
              Save draft
            </Button>
            <Button onClick={() => submit(true)} disabled={working}>
              Submit for approval
            </Button>
          </DialogFooter>
        </DialogContent>
