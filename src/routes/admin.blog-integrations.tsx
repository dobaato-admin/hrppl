...
function BlogIntegrationsPage() {
  ...
  const { data: keysData } = useQuery({
    queryKey: ["blog-api-keys"],
    queryFn: () => listKeysFn(),
    enabled: isSuper,
  });
  const { data: hooksData } = useQuery({
    queryKey: ["blog-webhooks"],
    queryFn: () => listHooksFn(),
    enabled: isSuper,
  });
  ...
  async function generateKey() {
    ...
    try {
      const res = await createKeyFn({
        data: { name: newKeyName.trim(), scopes: ["posts:read", "posts:write"] },
      });
      setNewKey(res.key);
      setNewKeyName("");
  ...
  async function saveHook(e: React.FormEvent) {
    e.preventDefault();
    try {
      const res = await saveHookFn({
        data: {
          id: hookForm.id,
          name: hookForm.name,
          url: hookForm.url,
          events: hookForm.events as ("post.published" | "post.updated" | "post.deleted")[],
          active: hookForm.active,
        },
      });
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Issue keys to Blaze.ai, AutoSEO, Make, Zapier or your own scripts. Each key can read &
              write posts.
            </CardDescription>
          </CardHeader>
            ...
            <div className="flex flex-wrap gap-2 items-end">
              <div className="flex-1 min-w-[240px]">
                ...
                <Input
                  ...
                  placeholder="e.g. Blaze.ai – production"
                  maxLength={80}
                />
              </div>
                    ...
                    <TableRow key={k.id}>
                      ...
                      <TableCell>
                        {k.revoked_at ? (
                          ...
                          <Badge className="bg-status-done text-status-done-foreground">
                            Active
                          </Badge>
                        )}
                      </TableCell>
        ...
        <Card>
          <CardHeader className="flex flex-row items-start justify-between">
            <div>
              ...
              <CardDescription>
                Notify external systems when posts are published, updated or deleted. Signed with
                HMAC-SHA256.
              </CardDescription>
            </div>
            <Button onClick={openNewHook}>
        ...
        <Dialog open={hookOpen} onOpenChange={setHookOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{hookForm.id ? "Edit webhook" : "New webhook"}</DialogTitle>
            </DialogHeader>
            <form onSubmit={saveHook} className="space-y-3">
              ...
              <div>
                ...
                <Input
                  ...
                  required
                  type="url"
                  placeholder="https://hooks.example.com/hrppl"
                />
              </div>
              <div className="flex items-center gap-2">
                <Switch
                  checked={hookForm.active}
                  onCheckedChange={(v) => setHookForm({ ...hookForm, active: v })}
                />
                <Label>Active</Label>
              </div>
              {hookSecret && (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              What to give Blaze.ai, AutoSEO or any HTTP-capable tool.
            </CardDescription>
          </CardHeader>
...
{ "event": "post.published", "occurred_at": "...", "data": { "post": { ... } } }`}</pre>
              <p className="text-muted-foreground mt-1">
                Verify the signature with the secret shown when you create the webhook.
              </p>
            </div>
          </CardContent>
        </Card>
