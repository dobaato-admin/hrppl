...
import {
  ...
  listTrainingBundles,
  upsertTrainingBundle,
  deleteTrainingBundle,
  applyTrainingBundle,
  listDocumentRequestTemplates,
  upsertDocumentRequestTemplate,
...
export const Route = createFileRoute("/admin/templates")({
  ...
  component: () => (
    <AdminGate feature="org.templatesHub">
      <TemplatesHub />
    </AdminGate>
  ),
});
          ...
          <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Reusable templates for onboarding, training, document requests, and reviews.
              </p>
            </div>
            <Link to="/org">
...
function OnboardingTab() {
  ...
  >([]);
  const [editor, setEditor] = useState<{ open: boolean; tpl: any | null }>({
    open: false,
    tpl: null,
  });
  const [applyDlg, setApplyDlg] = useState<{ open: boolean; tplId: string | null }>({
    open: false,
    tplId: null,
  });
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setTemplates(r.templates as OnbTpl[]);
      setItems(r.items as OnbItem[]);
      setCourses(r.courses as OnbCourse[]);
    } catch (e: any) {
  ...
  async function onClone(t: OnbTpl) {
    ...
    if (!name) return;
    try {
      await clone({ data: { id: t.id, newName: name } });
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            Reusable hire-day checklists with tasks and attached training courses.
          </CardDescription>
        </div>
      ...
      <CardContent className="space-y-3">
        {templates.length === 0 && (
          <p className="text-sm text-muted-foreground">
            No templates yet. Create your first to get started.
          </p>
        )}
        {templates.map((t) => {
            ...
            >
              <div>
                <div className="font-medium flex items-center gap-2">
                  {t.name} {t.is_default && <Badge variant="secondary">Default</Badge>}{" "}
                  {!t.is_active && <Badge variant="outline">Inactive</Badge>}
                </div>
                <div className="text-xs text-muted-foreground">
                  {tItems.length} tasks · {tCourses.length} courses{" "}
                  {t.role_target && `· role: ${t.role_target}`}
                </div>
              </div>
              <div className="flex gap-2">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setApplyDlg({ open: true, tplId: t.id })}
                >
                ...
                <Button variant="outline" size="sm" onClick={() => onClone(t)}>
      ...
      <Dialog open={editor.open} onOpenChange={(o) => !o && setEditor({ open: false, tpl: null })}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editor.tpl?.id ? "Edit template" : "New template"}</DialogTitle>
          </DialogHeader>
          {editor.tpl && (
            <OnbEditor tpl={editor.tpl} setTpl={(t) => setEditor({ open: true, tpl: t })} />
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditor({ open: false, tpl: null })}>
              Cancel
            </Button>
      ...
      <ApplyOnboardingDialog
        open={applyDlg.open}
        tplId={applyDlg.tplId}
        employees={employees}
        apply={apply}
        onClose={() => setApplyDlg({ open: false, tplId: null })}
      />
    </Card>
  );
}
    ...
    setTpl({
      ...
      items: [
        ...tpl.items,
        {
          ...
          due_offset_days: 0,
          required: true,
          sort_order: tpl.items.length,
        },
      ],
    });
  }
  function rmItem(idx: number) {
    setTpl({ ...tpl, items: tpl.items.filter((_: any, i: number) => i !== idx) });
  }
  function setItem(idx: number, patch: any) {
    setTpl({
      ...tpl,
      items: tpl.items.map((it: any, i: number) => (i === idx ? { ...it, ...patch } : it)),
    });
  }
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label>Name</Label>
          <Input value={tpl.name} onChange={(e) => up("name", e.target.value)} />
        </div>
        <div>
          <Label>Role target (optional)</Label>
          <Input
            value={tpl.role_target ?? ""}
            onChange={(e) => up("role_target", e.target.value)}
      ...
      <div className="flex items-center gap-4">
        <label className="flex items-center gap-2 text-sm">
          <Checkbox checked={tpl.is_default} onCheckedChange={(c) => up("is_default", !!c)} />{" "}
          Default template
        </label>
        <label className="flex items-center gap-2 text-sm">
          <Checkbox checked={tpl.is_active} onCheckedChange={(c) => up("is_active", !!c)} /> Active
        </label>
      </div>
          ...
          {tpl.items.map((it: any, idx: number) => (
            <div key={idx} className="grid grid-cols-12 gap-2 items-end rounded border p-2">
              <div className="col-span-4">
                ...
                <Input value={it.title} onChange={(e) => setItem(idx, { title: e.target.value })} />
              </div>
              <div className="col-span-2">
                <Label className="text-xs">Owner</Label>
                <Select
                  value={it.owner_role}
                  onValueChange={(v) => setItem(idx, { owner_role: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="col-span-1 flex items-center">
                <label className="flex items-center gap-1 text-xs">
                  <Checkbox
                    checked={it.required}
                    onCheckedChange={(c) => setItem(idx, { required: !!c })}
                  />{" "}
                  Req
                </label>
              </div>
              <div className="col-span-1">
                <Button size="sm" variant="ghost" onClick={() => rmItem(idx)}>
                  <Trash2 className="h-3 w-3" />
                </Button>
              </div>
            </div>
      ...
      <p className="text-xs text-muted-foreground">
        Tip: To attach training courses to this onboarding template, use the Training Bundles tab
        and assign both to the employee.
      </p>
    </div>
  );
}

function ApplyOnboardingDialog({
  open,
  tplId,
  employees,
  apply,
  ...
  async function go() {
    ...
    try {
      const r = await apply({
        data: { template_id: tplId, employee_id: empId, start_date: startDate },
      });
      toast.success(`Applied. Enrolled in ${r.enrolled} course(s).`);
      onClose();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Apply onboarding template</DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={go} disabled={!empId}>
            Apply
          </Button>
        </DialogFooter>
      </DialogContent>
...
function TrainingTab() {
  ...
  const [applyDlg, setApplyDlg] = useState<{ open: boolean; bId: string | null }>({
    open: false,
    bId: null,
  });
  ...
  function openNew() {
    setEditor({
      open: true,
      b: { name: "", description: "", target_role: "", is_active: true, items: [] },
    });
  }
  function openEdit(b: any) {
    setEditor({
      open: true,
      b: { ...b, items: items.filter((i) => i.bundle_id === b.id).map((i) => ({ ...i })) },
    });
  }
  async function save() {
    try {
      await upsert({ data: editor.b });
      toast.success("Saved");
      setEditor({ open: false, b: null });
      refresh();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle className="text-base">Training bundles</CardTitle>
          <CardDescription>
            Group training courses and assign them to employees in one click.
          </CardDescription>
        </div>
        <Button size="sm" onClick={openNew}>
        ...
        {bundles.map((b) => {
          ...
          return (
            ...
            >
              <div>
                <div className="font-medium flex items-center gap-2">
                  {b.name} {!b.is_active && <Badge variant="outline">Inactive</Badge>}
                </div>
                <div className="text-xs text-muted-foreground">
                  {bi.length} courses {b.target_role && `· role: ${b.target_role}`}
                </div>
              </div>
              <div className="flex gap-2">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setApplyDlg({ open: true, bId: b.id })}
                >
                ...
                <Button variant="outline" size="sm" onClick={() => onDelete(b)}>
                  <Trash2 className="h-3 w-3" />
                </Button>
              </div>
            </div>
      ...
      <Dialog open={editor.open} onOpenChange={(o) => !o && setEditor({ open: false, b: null })}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editor.b?.id ? "Edit bundle" : "New bundle"}</DialogTitle>
          </DialogHeader>
          {editor.b && (
            <BundleEditor
              b={editor.b}
              setB={(b) => setEditor({ open: true, b })}
              courses={courses}
            />
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditor({ open: false, b: null })}>
              Cancel
            </Button>
      ...
      <ApplyBundleDialog
        ...
        employees={employees}
        apply={apply}
        onClose={() => setApplyDlg({ open: false, bId: null })}
      />
    </Card>
  );
}
...
function BundleEditor({ b, setB, courses }: { b: any; setB: (b: any) => void; courses: any[] }) {
  function add() {
    setB({
      ...
      items: [
        ...b.items,
        { course_id: "", due_offset_days: 14, required: true, sort_order: b.items.length },
      ],
    });
  }
  function rm(i: number) {
    setB({ ...b, items: b.items.filter((_: any, idx: number) => idx !== i) });
  }
  function patch(i: number, p: any) {
    setB({ ...b, items: b.items.map((x: any, idx: number) => (idx === i ? { ...x, ...p } : x)) });
  }
  return (
    <div className="space-y-3">
      <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input
            value={b.target_role ?? ""}
            onChange={(e) => setB({ ...b, target_role: e.target.value })}
          />
        </div>
        <div className="flex items-end">
          <label className="flex items-center gap-2 text-sm">
            <Checkbox
              checked={b.is_active}
              onCheckedChange={(c) => setB({ ...b, is_active: !!c })}
            />{" "}
            Active
          </label>
        </div>
          ...
          <div key={idx} className="grid grid-cols-12 gap-2 items-end mb-2">
            ...
            <div className="col-span-3">
              ...
              <Input
                type="number"
                value={it.due_offset_days}
                onChange={(e) => patch(idx, { due_offset_days: Number(e.target.value) })}
              />
            </div>
            <div className="col-span-1">
              <label className="flex items-center gap-1 text-xs">
                <Checkbox
                  checked={it.required}
                  onCheckedChange={(c) => patch(idx, { required: !!c })}
                />{" "}
                Req
              </label>
            </div>
            <div className="col-span-1">
              <Button variant="ghost" size="sm" onClick={() => rm(idx)}>
                <Trash2 className="h-3 w-3" />
              </Button>
            </div>
          </div>
...
function ApplyBundleDialog({
  ...
  async function go() {
    ...
    try {
      const r = await apply({ data: { bundle_id: bId, employee_ids: ids, start_date: startDate } });
      toast.success(`Enrolled ${r.enrolled} entries.`);
      onClose();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Assign training bundle</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div>
            <Label>Start date</Label>
              ...
              {employees.map((e) => (
                <label key={e.id} className="flex items-center gap-2 text-sm">
                  <Checkbox
                    ...
                    onCheckedChange={(c) => setSelected({ ...selected, [e.id]: !!c })}
                  />
        ...
        <DialogFooter>
          ...
          <Button onClick={go} disabled={count === 0}>
            Assign to {count}
          </Button>
        </DialogFooter>
      </DialogContent>
...
function DocumentsTab() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setTpls(r.templates);
      setItems(r.items);
      setDocTpls(r.documentTemplates);
    } catch (e: any) {
      toast.error(e.message);
    }
  }, [list]);
  ...
  function openNew() {
    setEditor({
      open: true,
      t: { name: "", description: "", trigger: "onboarding", is_active: true, items: [] },
    });
  }
  function openEdit(t: any) {
    setEditor({
      open: true,
      t: { ...t, items: items.filter((i) => i.template_id === t.id).map((i) => ({ ...i })) },
    });
  }
  async function save() {
    try {
      await upsert({ data: editor.t });
      toast.success("Saved");
      setEditor({ open: false, t: null });
      refresh();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle className="text-base">Document request bundles</CardTitle>
          <CardDescription>
            Group documents to request together — e.g. on hire, promotion, or offboarding.
          </CardDescription>
        </div>
        <Button size="sm" onClick={openNew}>
          <Plus className="mr-1 h-4 w-4" /> New bundle
        ...
        {tpls.map((t) => {
          ...
          return (
            ...
            >
              <div>
                <div className="font-medium flex items-center gap-2">
                  {t.name} {t.trigger && <Badge variant="secondary">{t.trigger}</Badge>}{" "}
                  {!t.is_active && <Badge variant="outline">Inactive</Badge>}
                </div>
      ...
      <Dialog open={editor.open} onOpenChange={(o) => !o && setEditor({ open: false, t: null })}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editor.t?.id ? "Edit bundle" : "New bundle"}</DialogTitle>
          </DialogHeader>
          {editor.t && (
            <DocBundleEditor
              t={editor.t}
              setT={(t) => setEditor({ open: true, t })}
              docTpls={docTpls}
            />
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditor({ open: false, t: null })}>
              Cancel
            </Button>
            <Button onClick={save}>Save</Button>
          </DialogFooter>
    ...
    setT({
      ...
      items: [
        ...t.items,
        {
          ...
          due_offset_days: 7,
          sort_order: t.items.length,
        },
      ],
    });
  }
  function rm(i: number) {
    setT({ ...t, items: t.items.filter((_: any, idx: number) => idx !== i) });
  }
  function patch(i: number, p: any) {
    setT({ ...t, items: t.items.map((x: any, idx: number) => (idx === i ? { ...x, ...p } : x)) });
  }
  return (
    <div className="space-y-3">
      <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="flex items-end">
          <label className="flex items-center gap-2 text-sm">
            <Checkbox
              checked={t.is_active}
              onCheckedChange={(c) => setT({ ...t, is_active: !!c })}
            />{" "}
            Active
          </label>
        </div>
      ...
      <div>
        ...
        {t.items.map((it: any, idx: number) => (
          <div key={idx} className="grid grid-cols-12 gap-2 items-end mb-2">
            <div className="col-span-7">
              <Label className="text-xs">Document template</Label>
              <Select
                value={it.document_template_id}
                onValueChange={(v) => patch(idx, { document_template_id: v })}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select…" />
                </SelectTrigger>
            ...
            <div className="col-span-2">
              <label className="flex items-center gap-1 text-xs">
                <Checkbox
                  checked={it.required_signature}
                  onCheckedChange={(c) => patch(idx, { required_signature: !!c })}
                />{" "}
                E-sign
              </label>
            </div>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Design rating scales, competencies, and section sets. Clone any template and save as a new
          version.
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <Link to="/admin/review-templates">
          <Button>Open review templates →</Button>
        </Link>
      </CardContent>
    </Card>
  );
