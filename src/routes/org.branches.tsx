...
interface HolidayCategory { id: string; name: string; country_code: string }

const empty: BranchForm = {
  name: "", code: "", country_code: "", currency_code: "", timezone: "",
  address_line1: "", address_line2: "", city: "", region: "", postal_code: "",
  contact_name: "", contact_email: "", contact_phone: "",
  holiday_category_id: "", is_headquarters: false, status: "active",
};

interface BranchForm {
  name: string; code: string; country_code: string; currency_code: string; timezone: string;
  address_line1: string; address_line2: string; city: string; region: string; postal_code: string;
  contact_name: string; contact_email: string; contact_phone: string;
  holiday_category_id: string; is_headquarters: boolean; status: "active" | "inactive";
}
...
function BranchesPage() {
  ...
  async function refresh() {
    ...
    const [b, c, h] = await Promise.all([
      tid ? supabase.from("tenant_branches").select("*").eq("tenant_id", tid).order("is_headquarters", { ascending: false }).order("name") : Promise.resolve({ data: [] as Branch[] }),
  ...
  useEffect(() => { if (rolesLoaded && user) refresh(); }, [rolesLoaded, user]);
  ...
  function openEdit(b: Branch) {
    setEditing(b);
    setForm({
      name: b.name, code: b.code, country_code: b.country_code, currency_code: b.currency_code,
      timezone: b.timezone ?? "", address_line1: b.address_line1 ?? "", address_line2: b.address_line2 ?? "",
      city: b.city ?? "", region: b.region ?? "", postal_code: b.postal_code ?? "",
      ...
      is_headquarters: b.is_headquarters, status: b.status,
    });
    setOpen(true);
  }
  ...
  async function remove(b: Branch) {
    ...
    if (attached > 0) {
      ...
      toast.success(`Moved ${attached} staff to ${target.name}. Affected staff will be notified by email.`);
    } else {
  ...
  return (
    <AppShell
      ...
      actions={
        canManage ? (
          <Button onClick={openCreate} size="sm"><Plus className="mr-2 h-4 w-4" />New branch</Button>
        ) : null
      }
    >
      <section className="mx-auto max-w-6xl px-6 py-8 space-y-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Building2 className="h-4 w-4" /> All branches ({branches.length})</CardTitle>
            <CardDescription>
              Branches can be in any country. The headquarters branch is highlighted.{" "}
              <Link to="/org" className="underline">Back to organization</Link>
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
                ...
                {branches.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={8} className="text-muted-foreground text-center py-8">
                      No branches yet. {canManage && "Click \"New branch\" to add your first location."}
                    </TableCell>
                  </TableRow>
                ) : branches.map((b) => (
                  <TableRow key={b.id}>
                    <TableCell className="font-medium">
                      {b.name}
                      {b.is_headquarters && <Badge variant="default" className="ml-2">HQ</Badge>}
                    </TableCell>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Branches can be located anywhere — same country or different.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-2 md:grid-cols-2">
            <div className="space-y-2 md:col-span-1">
              <Label>Name *</Label>
              <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Sydney HQ" />
            </div>
            <div className="space-y-2 md:col-span-1">
              <Label>Code *</Label>
              <Input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="e.g. SYD" />
            </div>
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => {
                  ...
                  setForm({ ...form, country_code: v, currency_code: c?.currency_code ?? form.currency_code });
                }}
              >
            ...
            <div className="space-y-2">
              ...
              <Input value={form.currency_code} onChange={(e) => setForm({ ...form, currency_code: e.target.value.toUpperCase() })} placeholder="AUD" />
            </div>
            <div className="space-y-2 md:col-span-2">
              <Label>Timezone</Label>
              <Select value={form.timezone || "__unset"} onValueChange={(v) => setForm({ ...form, timezone: v === "__unset" ? "" : v })}>
            ...
            <div className="space-y-2"><Label>State / Region</Label><Input value={form.region} onChange={(e) => setForm({ ...form, region: e.target.value })} /></div>
            ...
            <div className="space-y-2"><Label>Public holiday category</Label>
              <Select value={form.holiday_category_id || "__none"} onValueChange={(v) => setForm({ ...form, holiday_category_id: v === "__none" ? "" : v })}>
                <SelectTrigger><SelectValue placeholder="None" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="__none">None</SelectItem>
                  {matchingHolidayCats.map((h) => <SelectItem key={h.id} value={h.id}>{h.name} ({h.country_code})</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex items-center gap-3 md:col-span-2 pt-2">
              ...
              <Label htmlFor="hq" className="cursor-pointer">This is the headquarters branch</Label>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button>
            <Button onClick={save} disabled={saving}>{saving ? "Saving…" : editing ? "Save changes" : "Create branch"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </AppShell>
