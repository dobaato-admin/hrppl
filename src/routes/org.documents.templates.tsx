...
import { listTemplates, upsertTemplate, publishTemplate, archiveTemplate, cloneTemplate, getTemplate, renderTemplatePreview, listStarterTemplates, instantiateStarterTemplate } from "@/lib/documents.functions";
...
const MERGE_HINTS = [
  ...
  "employee.email", "employee.job_title", "company.name", "today",
];
...
function TemplatesPage() {
  ...
  async function refresh() { setRows((await list()).templates); }
  useEffect(() => { refresh(); }, []);

  async function openStarters() {
    setStarterOpen(true);
    if (starters.length === 0) {
      try { setStarters((await listStarters()).presets); }
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-3">
        <div>
          ...
          <CardDescription>Author contracts, offers, policies and HR letters with merge tags.</CardDescription>
        </div>
        <div className="flex gap-2">
          <Button size="sm" variant="outline" onClick={openStarters}>
            <Sparkles className="h-4 w-4 mr-1" /> Browse starters
          </Button>
          <Button size="sm" onClick={() => setEditor({ name: "", doc_type: "employment_contract", body_html: "", merge_fields: [], requires_signature: true, requires_countersign: false, default_due_days: 14 })}>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Version</TableHead><TableHead>Updated</TableHead>
              <TableHead className="text-right">Actions</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {rows.length === 0 ? (
              <TableRow><TableCell colSpan={6} className="text-muted-foreground">No templates yet.</TableCell></TableRow>
            ) : rows.map((t) => (
              <TableRow key={t.id}>
                <TableCell className="font-medium">{t.name}</TableCell>
                <TableCell><Badge variant="outline">{t.doc_type.replace(/_/g, " ")}</Badge></TableCell>
                <TableCell>
                  ...
                  {t.status === "draft" && (
                    <>
                      <Button size="sm" variant="ghost" onClick={async () => {
                        ...
                        setEditor({ ...r.template, merge_fields: r.template.merge_fields ?? [] });
                      }}><Pencil className="h-3.5 w-3.5" /></Button>
                      <Button size="sm" variant="ghost" onClick={async () => {
                        await publish({ data: { id: t.id } });
                        toast.success("Published"); refresh();
                      }}><CheckCircle2 className="h-3.5 w-3.5" /></Button>
                    </>
                  )}
                  <Button size="sm" variant="ghost" onClick={async () => {
                    await clone({ data: { id: t.id } });
                    toast.success("Cloned"); refresh();
      ...
      <Dialog open={starterOpen} onOpenChange={setStarterOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Sparkles className="h-4 w-4" /> HRPPL recommended starters</DialogTitle>
            <CardDescription>Add a starter to your library as a draft. You can rename, edit, and publish it for your organisation.</CardDescription>
          </DialogHeader>
            ...
            {starters.length === 0 ? (
              ...
              <div key={p.key} className="flex flex-col gap-2 rounded-md border p-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <Badge variant="outline" className="shrink-0 text-[10px]">{p.doc_type.replace(/_/g, " ")}</Badge>
                </div>
                <div className="flex flex-wrap gap-1 text-[10px] text-muted-foreground">
                  {p.requires_signature && <span className="rounded bg-muted px-1.5 py-0.5">Signature</span>}
                  {p.requires_countersign && <span className="rounded bg-muted px-1.5 py-0.5">Countersign · {p.countersigner_role}</span>}
                  <span className="rounded bg-muted px-1.5 py-0.5">Due {p.default_due_days}d</span>
                </div>
                <Button size="sm" disabled={instantiating === p.key} onClick={async () => {
      ...
      <Dialog open={!!editor} onOpenChange={(o) => { if (!o) setEditor(null); }}>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {editor && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-2">
                  <Label>Type</Label>
                  <Select value={editor.doc_type} onValueChange={(v) => setEditor({ ...editor, doc_type: v })}>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="flex items-center gap-2">
                  <Switch checked={editor.requires_signature} onCheckedChange={(v) => setEditor({ ...editor, requires_signature: v })} />
                  <Label>Requires signature</Label>
                </div>
                <div className="flex items-center gap-2">
                  <Switch checked={editor.requires_countersign} onCheckedChange={(v) => setEditor({ ...editor, requires_countersign: v })} />
                  <Label>Countersign</Label>
                </div>
                <div className="space-y-1">
                  <Label className="text-xs">Default due (days)</Label>
                  <Input type="number" min={1} max={365} value={editor.default_due_days}
                    onChange={(e) => setEditor({ ...editor, default_due_days: Number(e.target.value) })} />
                </div>
              </div>
              {editor.requires_countersign && (
                <div className="space-y-2">
                  <Label>Countersigner role (label shown to admins)</Label>
                  <Input value={editor.countersigner_role ?? ""} placeholder="e.g. HR Manager, CEO"
          ...
          <DialogFooter>
            <Button disabled={working || !editor?.name || !editor?.body_html} onClick={async () => {
              setWorking(true);
              try {
                await upsert({ data: {
                  id: editor.id, name: editor.name, description: editor.description ?? null,
                  doc_type: editor.doc_type, body_html: editor.body_html,
            ...
            }}>{working ? "Saving…" : "Save draft"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </Card>
...
function TemplateBodyEditor({ bodyHtml, onChange }: { bodyHtml: string; onChange: (html: string) => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <Tabs value={tab} onValueChange={(v) => { setTab(v as any); if (v === "preview") runPreview(); }}>
        <TabsList>
          <TabsTrigger value="edit"><Pencil className="h-3.5 w-3.5 mr-1" /> Edit</TabsTrigger>
          <TabsTrigger value="preview"><Eye className="h-3.5 w-3.5 mr-1" /> Preview merged</TabsTrigger>
        </TabsList>
        <TabsContent value="edit" className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1 text-xs">
            ...
            {MERGE_HINTS.map((m) => (
              <button key={m} type="button" className="rounded bg-muted px-1.5 py-0.5 font-mono hover:bg-accent"
              ...
              {preview.unresolved.length > 0 && (
                <div className="flex items-start gap-2 rounded-md border border-status-working/40 bg-status-working/10 p-2 text-xs">
                  ...
                  <div>
                    ...
                    <div className="text-muted-foreground">{preview.unresolved.map((u) => `{{${u}}}`).join(", ")}</div>
                  </div>
                </div>
              )}
              <div className="rounded-md border bg-card p-4 prose prose-sm dark:prose-invert max-h-[420px] overflow-y-auto"
                dangerouslySetInnerHTML={{ __html: preview.html }} />
              <div className="text-[10px] text-muted-foreground">Sample values used: employee.full_name=Alex Sample · company.name=Acme Inc. · today=today</div>
            </>
          ) : null}
        </TabsContent>
      </Tabs>
