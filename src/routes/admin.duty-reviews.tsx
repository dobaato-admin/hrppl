...
import { getDutyReview, upsertDutyScore, exportDutyReviewCsv, getDutyReviewExportData } from "@/lib/duty-reviews.functions";
...
function DutyReviewsPage() {
  ...
  async function downloadPdf() {
    try {
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = `duty-review-${cycleLabel}.pdf`; a.click();
      URL.revokeObjectURL(url);
  ...
  useEffect(() => {
    ...
    for (const it of reviewQ.data.items) {
      map[it.duty.id] = { score: it.score == null ? "" : String(it.score), comments: it.comments ?? "" };
    }
    setDrafts(map);
  }, [reviewQ.data]);
  ...
  async function saveOne(dutyId: string) {
    ...
    if (Number.isNaN(score) || score < 0 || score > 100) { toast.error("Score must be 0–100"); return; }
    try {
  ...
  async function saveAll() {
    ...
    try {
      for (const it of reviewQ.data.items) {
        ...
        await saveFn({ data: { employeeId, dutyId: it.duty.id, cycleLabel, score, comments: d.comments || "" } });
      }
  ...
  return (
    <AppShell title="Duty-based KPI review" subtitle="Score each duty out of 100. Final rating is weighted by KPI weight.">
      <section className="mx-auto max-w-5xl space-y-6 p-4 md:p-6">
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2"><Target className="h-5 w-5 text-primary" /><CardTitle>Pick employee &amp; cycle</CardTitle></div>
            <CardDescription>Cycle label is free-form (e.g. 2026-Q1, 2026 H1, 2026 annual).</CardDescription>
          </CardHeader>
              ...
              <Select value={cycleLabel} onValueChange={setCycleLabel}>
                ...
                <SelectContent>
                  {(cyclesQ.data?.cycles ?? []).map((c: any) => (
                    <SelectItem key={c.id} value={c.label}>{c.label} · {c.status}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div className="flex flex-wrap items-end justify-end gap-2">
              <Button variant="outline" onClick={downloadCsv} disabled={!cycleLabel}><Download className="h-4 w-4 mr-1" />Export CSV</Button>
              <Button variant="outline" onClick={downloadPdf} disabled={!cycleLabel}><FileText className="h-4 w-4 mr-1" />Export PDF</Button>
              <Button onClick={saveAll} disabled={!reviewQ.data || reviewQ.data.items.length === 0}>Save all</Button>
            </div>
          </CardContent>
        </Card>
        ...
        {employeeId && reviewQ.data && (
          <Card>
            <CardHeader>
              <CardTitle>{reviewQ.data.employee.first_name} {reviewQ.data.employee.last_name}</CardTitle>
              <CardDescription>
                {reviewQ.data.items.length} duties · Total weight {reviewQ.data.totalWeight}%
                {liveFinal != null && <> · <Badge variant={liveFinal >= 70 ? "default" : liveFinal >= 50 ? "secondary" : "outline"}>Final {liveFinal.toFixed(1)}/100</Badge></>}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {reviewQ.data.items.length === 0 && (
                <p className="text-sm text-muted-foreground">This employee has no active duties yet. Add them from Duties &amp; responsibilities.</p>
              )}
              {reviewQ.data.items.map((it: any) => (
                <div key={it.duty.id} className="rounded border p-3 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="font-medium">{it.duty.title} <Badge variant="outline" className="ml-1">{it.duty.weight}%</Badge></div>
                      {it.duty.kpi_target && <div className="text-xs text-muted-foreground">Target: {it.duty.kpi_target}</div>}
                      {it.duty.description && <div className="text-xs text-muted-foreground">{it.duty.description}</div>}
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-[120px_1fr_auto] gap-2 items-start">
                    ...
                    <div className="pt-5">
                      <Button size="sm" variant="outline" onClick={() => saveOne(it.duty.id)}>Save</Button>
                    </div>
                  </div>
                </div>
              ))}
