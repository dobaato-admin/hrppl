...
interface Timesheet { id: string; employee_id: string; period_start: string; period_end: string; total_hours: number; overtime_hours: number; status: string; submitted_at: string | null; rejection_reason: string | null; overtime_breakdown: Record<string, number> | null }
interface Emp { id: string; first_name: string; last_name: string; employee_number: string }
interface PenaltyRate { id: string; code: string; name: string; rate_multiplier: number; effective_from: string; effective_to: string | null; is_active: boolean }
...
function OrgTimesheets() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (prof?.tenant_id) {
        ...
        const { data: t } = await supabase.from("tenants").select("country_code").eq("id", prof.tenant_id).maybeSingle();
  ...
  async function load() {
    ...
    const [tRes, eRes] = await Promise.all([
      supabase.from("timesheets").select("*").eq("tenant_id", tenantId).order("period_start", { ascending: false }),
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase.from("overtime_penalty_rates").select("*")
        .eq("country_code", countryCode).eq("is_active", true).order("code");
  ...
  async function onApprove() {
    ...
    try {
      ...
      if (anyBreakdownEntered) {
        ...
        await supabase.from("timesheets").update({ overtime_breakdown: {} }).eq("id", approveTarget.id);
      }
      ...
      <TableRow key={r.id}>
        ...
        {actions && (
          <TableCell className="text-right space-x-1">
            <Button size="sm" disabled={busy} onClick={() => openApprove(r)}>Approve</Button>
            <Button size="sm" variant="outline" disabled={busy} onClick={() => { setRejectTarget(r); setReason(""); }}>Reject</Button>
          </TableCell>
        )}
      </TableRow>
    );
  ...
  return (
    <main className="min-h-screen bg-background">
        ...
        <Tabs defaultValue="submitted">
          ...
          <TabsContent value="submitted">
            <Card>
              ...
              <CardContent>
                <Table>
                  <TableHeader><TableRow><TableHead>Employee</TableHead><TableHead>Period</TableHead><TableHead>Hours</TableHead><TableHead>OT</TableHead><TableHead>Status</TableHead><TableHead></TableHead></TableRow></TableHeader>
                  <TableBody>
                    {submitted.map((r) => <Row key={r.id} r={r} actions />)}
                    {submitted.length === 0 && <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground">Nothing to approve.</TableCell></TableRow>}
                  </TableBody>
                </Table>
              </CardContent>
            </Card>
          ...
          <TabsContent value="history">
            <Card>
              ...
              <CardContent>
                <Table>
                  <TableHeader><TableRow><TableHead>Employee</TableHead><TableHead>Period</TableHead><TableHead>Hours</TableHead><TableHead>OT</TableHead><TableHead>Status</TableHead></TableRow></TableHeader>
                  <TableBody>
                    {history.map((r) => <Row key={r.id} r={r} />)}
                    {history.length === 0 && <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground">No history.</TableCell></TableRow>}
                  </TableBody>
                </Table>
              </CardContent>
            </Card>
      ...
      <Dialog open={!!approveTarget} onOpenChange={(o) => { if (!o) { setApproveTarget(null); setBreakdown({}); } }}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>Approve timesheet</DialogTitle></DialogHeader>
          {approveTarget && (
            <div className="space-y-3 text-sm">
              <div className="text-muted-foreground">
                Total overtime: <span className="font-medium text-foreground">{Number(approveTarget.overtime_hours).toFixed(2)} h</span>
              </div>
              {activeRatesForTarget.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  No overtime penalty rates configured for this country. The country default multiplier will be used.
                </p>
              ) : (
                <>
                  <div className="text-xs text-muted-foreground">
                    Optionally split overtime hours across penalty rates. Leave all empty to use the country default multiplier.
                  </div>
                    ...
                    {activeRatesForTarget.map((rate) => (
                      <div key={rate.code} className="grid grid-cols-[1fr_auto] items-center gap-2">
                        <Label className="text-xs">
                          ...
                          <span>{rate.name}</span>
                          <span className="text-muted-foreground"> · ×{Number(rate.rate_multiplier).toFixed(2)}</span>
                        </Label>
                        <Input
                          type="number" step="0.01" min={0} className="w-24"
                          value={breakdown[rate.code] ?? ""}
                          onChange={(e) => setBreakdown({ ...breakdown, [rate.code]: e.target.value })}
                        />
                      </div>
                    ))}
                  </div>
                  {anyBreakdownEntered && (
                    <div className={`text-xs ${breakdownMatches ? "text-muted-foreground" : "text-destructive"}`}>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setApproveTarget(null); setBreakdown({}); }}>Cancel</Button>
            <Button onClick={onApprove} disabled={approveDisabled}>Approve</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!rejectTarget} onOpenChange={(o) => { if (!o) setRejectTarget(null); }}>
        <DialogContent>
          ...
          <Textarea rows={4} placeholder="Reason (optional)" value={reason} onChange={(e) => setReason(e.target.value)} />
          <DialogFooter>
