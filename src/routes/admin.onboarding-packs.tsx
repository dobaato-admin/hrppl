...
export const Route = createFileRoute("/admin/onboarding-packs")({
  ...
  component: () => (
    <AdminGate feature="org.onboardingPacks">
      <OnboardingPacksPage />
    </AdminGate>
  ),
});
...
function OnboardingPacksPage() {
  ...
  const [cloneScope, setCloneScope] = useState<{
    country?: string | null;
    branch?: string | null;
    dept?: string | null;
    emp?: string | null;
  ...
  const branches: { id: string; name: string; country_code: string | null }[] = (data?.branches ??
    []) as any;
  ...
  const filtered = useMemo(() => {
    return packs.filter((p) => {
      if (
        filterCountry !== ANY &&
        (p.country_code ?? "") !== (filterCountry === "" ? "" : filterCountry)
      )
        return false;
      if (filterBranch !== ANY && (p.branch_id ?? "") !== (filterBranch === "" ? "" : filterBranch))
        return false;
      if (filterDept !== ANY && (p.department_id ?? "") !== (filterDept === "" ? "" : filterDept))
        return false;
      if (filterEmp !== ANY && (p.employment_type ?? "") !== (filterEmp === "" ? "" : filterEmp))
        return false;
      return true;
    });
  ...
  function openEdit(p: Pack) {
    setEditing({ ...p, items: [...(p.items ?? [])], stages: [...(p.stages ?? [])] });
    setEditorOpen(true);
  }
  function openClone(p: Pack) {
    setCloneSource(p);
    setCloneName(`${p.name} (override)`);
    setCloneScope({
  ...
  async function doDelete(p: Pack) {
    if (p.is_system_seed) {
      toast.error("Clone the seed first, then edit the override.");
      return;
    }
  ...
  return (
    <AppShell
      title="Onboarding & offboarding packs"
      subtitle="Override and reorder pre-loaded country packs per branch, department, and employment type"
    >
      <div className="p-4 space-y-4">
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              Scope what you see and what new packs inherit by default.
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              {isLoading
                ? "Loading…"
                : `${filtered.length} pack${filtered.length === 1 ? "" : "s"} in current scope`}{" "}
                ...
                {filtered.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center text-muted-foreground py-8">
                      No packs in this scope. Clone a system seed to create an override.
                    </TableCell>
                  </TableRow>
                )}
                {filtered.map((p, i) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="text-xs">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {p.branch_id && (
                          <Badge variant="outline">{branchMap.get(p.branch_id) ?? "Branch"}</Badge>
                        )}
                        {p.department_id && (
                          <Badge variant="outline">{deptMap.get(p.department_id) ?? "Dept"}</Badge>
                        )}
                        {p.employment_type && <Badge variant="outline">{p.employment_type}</Badge>}
                      </div>
                    </TableCell>
                    <TableCell className="text-center">
                      <Badge>{p.items?.length ?? 0}</Badge>
                    </TableCell>
                    ...
                    <TableCell className="text-right space-x-1 whitespace-nowrap">
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() => openClone(p)}
                        title="Clone as editable override"
                      >
                      ...
                      <Button
                        ...
                        variant="ghost"
                        onClick={() => openEdit(p)}
                        disabled={p.is_system_seed}
                        title={p.is_system_seed ? "Clone seeds before editing" : "Edit"}
                      >
                        Edit
                      </Button>
                      <Button
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base">Offboarding packs</CardTitle>
            <CardDescription>
              Per-country offboarding templates including comms-channel removal steps are managed in
              the offboarding admin.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button variant="outline" onClick={() => navigate({ to: "/admin/offboarding" })}>
              Open offboarding templates →
            </Button>
          </CardContent>
        </Card>
      </div>
      ...
      <Dialog open={editorOpen} onOpenChange={setEditorOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <Label className="text-xs">Country</Label>
                  <Select
                    value={editing.country_code ?? ANY}
                    onValueChange={(v) =>
                      setEditing({ ...editing, country_code: v === ANY ? null : v })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                ...
                <div>
                  <Label className="text-xs">Branch</Label>
                  <Select
                    value={editing.branch_id ?? ANY}
                    onValueChange={(v) =>
                      setEditing({ ...editing, branch_id: v === ANY ? null : v })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                ...
                <div>
                  <Label className="text-xs">Department</Label>
                  <Select
                    value={editing.department_id ?? ANY}
                    onValueChange={(v) =>
                      setEditing({ ...editing, department_id: v === ANY ? null : v })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                ...
                <div>
                  <Label className="text-xs">Employment type</Label>
                  <Select
                    value={editing.employment_type ?? ANY}
                    onValueChange={(v) =>
                      setEditing({ ...editing, employment_type: v === ANY ? null : v })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Label className="text-xs">Default pack for this scope</Label>
              </div>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setEditing({
                        ...
                        items: [
                          ...editing.items,
                          { key: "", label: "New step", required: true, stage: null },
                        ],
                      })
                    }
                  >
                    ...
                    <div key={i} className="flex items-center gap-2 p-2 border rounded">
                      <div className="flex flex-col">
                        <Button
                          ...
                          aria-label="Move up"
                          onClick={() => {
                            const a = [...editing.items];
                            [a[i - 1], a[i]] = [a[i], a[i - 1]];
                            setEditing({ ...editing, items: a });
                          }}
                        >
                          <ArrowUp className="w-3 h-3" />
                        </Button>
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          const a = [...editing.items];
                          a[i] = { ...a[i], stage: e.target.value || null };
                          setEditing({ ...editing, items: a });
                        }}
                      />
          ...
          <DialogFooter>
            ...
            {editing?.id && (
              <Button variant="outline" onClick={() => editing && exportPdf(editing as any)}>
      ...
      <Dialog open={cloneOpen} onOpenChange={setCloneOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Clone as override</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <Label className="text-xs">Country</Label>
                <Select
                  value={cloneScope.country ?? ANY}
                  onValueChange={(v) =>
                    setCloneScope({ ...cloneScope, country: v === ANY ? null : v })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div>
                <Label className="text-xs">Branch</Label>
                <Select
                  value={cloneScope.branch ?? ANY}
                  onValueChange={(v) =>
                    setCloneScope({ ...cloneScope, branch: v === ANY ? null : v })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div>
                <Label className="text-xs">Department</Label>
                <Select
                  value={cloneScope.dept ?? ANY}
                  onValueChange={(v) =>
                    setCloneScope({ ...cloneScope, dept: v === ANY ? null : v })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div>
                <Label className="text-xs">Employment type</Label>
                <Select
                  value={cloneScope.emp ?? ANY}
                  onValueChange={(v) => setCloneScope({ ...cloneScope, emp: v === ANY ? null : v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
