          ...
          <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Configure leave categories your employees can request.
              </p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Dialog open={createOpen} onOpenChange={setCreateOpen}>
                <DialogTrigger asChild>
                  <Button size="sm">New leave type</Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Create leave type</DialogTitle>
                    <DialogDescription>
                      Define a leave category for your organization.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="space-y-3">
                        ...
                        {PRESET_COLORS.map((c) => (
                          <button
                            ...
                            style={{
                              background: c,
                              borderColor:
                                form.color === c ? "hsl(var(--foreground))" : "transparent",
                            }}
                          />
                        ))}
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setForm({ ...form, annual_quota_days: Number(e.target.value) })
                          }
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          type="number"
                          step="0.01"
                          value={form.accrual_per_month}
                          onChange={(e) =>
                            setForm({ ...form, accrual_per_month: Number(e.target.value) })
                          }
                        />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div className="flex items-center justify-between">
                        ...
                        <Switch
                          checked={form.allow_carry_over}
                          onCheckedChange={(v) => setForm({ ...form, allow_carry_over: v })}
                        />
                      </div>
                      <div>
                        <Label>Max carry-over days (0 = no cap)</Label>
                        <Input
                          type="number"
                          step="0.5"
                          value={form.max_carry_over_days}
                          onChange={(e) =>
                            setForm({ ...form, max_carry_over_days: Number(e.target.value) })
                          }
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <Select
                        value={form.branch_id ?? "__all"}
                        onValueChange={(v) =>
                          setForm({ ...form, branch_id: v === "__all" ? null : v })
                        }
                      >
                        <SelectTrigger>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        Limit this leave type to a specific branch, or leave it open to everyone.
                      </p>
                    </div>
                  </div>
                  <DialogFooter>
        ...
        <section className="mx-auto max-w-5xl px-6 py-8">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{types.length} type(s)</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  ...
                  {types.map((t) => (
                    <TableRow key={t.id}>
                      ...
                      <TableCell>
                        {t.allow_carry_over ? (
                          <Badge variant="outline">
                            {Number(t.max_carry_over_days) > 0
                              ? `${t.max_carry_over_days}d cap`
                              : "no cap"}
                          </Badge>
                        ) : (
                      ...
                      <TableCell>
                        <Switch
                          checked={t.is_paid}
                          onCheckedChange={(v) => patch(t.id, { is_paid: v })}
                        />
                      </TableCell>
                      <TableCell>
                        <Switch
