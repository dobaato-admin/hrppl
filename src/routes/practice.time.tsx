...
import { allocationRollup } from "@/lib/timesheet-allocations.functions";
...
function TimePage() {
  ...
  // W5 P3 · allocationRollup had no caller, so time could be allocated to
  // projects and cost centres and then never read back in aggregate. "How many
  // hours went to which project" is the question the allocation feature exists
  // to answer, and it was only answerable one entry at a time.
  const rollupFn = useServerFn(allocationRollup);
  const { data: rollup } = useQuery({
    queryKey: ["allocation-rollup"],
    queryFn: () => rollupFn({ data: {} }),
    staleTime: 60_000,
  });
  const topProjects = Object.entries(rollup?.byProject ?? {})
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([id, hours]) => ({
      hours,
      name: (pd?.projects ?? []).find((p: any) => p.id === id)?.name ?? "Unassigned",
    }));
  ...
  const totalThisWeek = (data?.entries ?? [])
    .filter((e: any) => {
  ...
  async function submitForApproval() {
    try {
      await submitFn({
        data: { period_start: periodStart, period_end: periodEnd, notes: submitNotes || null },
      });
            ...
            <CardContent className="grid gap-3 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm({ ...form, project_id: v, job_id: "" })}
                >
                  <SelectTrigger>
              ...
              <div className="sm:col-span-2">
                <Button onClick={submit} disabled={!form.project_id || !form.hours}>
                  Log time
                </Button>
              </div>
            </CardContent>
                    ...
                    <TableRow key={e.id}>
                      ...
                      <TableCell className="text-right">
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => setAllocFor({ id: e.id, hours: Number(e.hours) })}
                        >
