...
function PayrollSettingsPage() {
  ...
  useEffect(() => {
    const allowed = roles.includes("super_admin") || roles.includes("regional_admin") || roles.includes("org_admin");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (isOrg) {
        ...
        const { data } = await supabase.from("countries").select("*").eq("code", tenant.country_code).order("name");
        setCountries((data ?? []) as Country[]);
        if (data && data.length > 0 && !selected) setSelected(data[0].code as string);
        return;
      }
      ...
      if (isRegional) {
        ...
        const { data } = await supabase.from("countries").select("*").in("code", codes).order("name");
        setCountries((data ?? []) as Country[]);
        if (data && data.length > 0 && !selected) setSelected(data[0].code as string);
      }
    })();
  ...
  async function loadCountry(code: string) {
    const [{ data: d }, { data: b }, { data: c }] = await Promise.all([
      ...
      supabase.from("tax_brackets").select("*").eq("country_code", code).order("effective_from", { ascending: false }).order("bracket_order"),
      supabase.from("contribution_rules").select("*").eq("country_code", code).order("effective_from", { ascending: false }),
    ]);
  ...
  useEffect(() => { if (selected) loadCountry(selected); }, [selected]);
  ...
  const canView = roles.includes("super_admin") || roles.includes("regional_admin") || roles.includes("org_admin");
      ...
      <header className="border-b border-border bg-card">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Country-level tax, contributions, and defaults</p>
          </div>
          <Link to={isSuper ? "/admin" : "/org"}><Button variant="outline" size="sm">Back</Button></Link>
        </div>
      </header>
          ...
          <CardContent>
            <div className="flex items-center gap-3">
              <Select value={selected} onValueChange={setSelected}>
                <SelectTrigger className="max-w-sm"><SelectValue placeholder="Select country" /></SelectTrigger>
                <SelectContent>
                  {countries.map((c) => (
      ...
      <CardContent>
        ...
        >
          <Field label="Pay frequency">
            <Select value={form.pay_frequency} onValueChange={(v) => setForm({ ...form, pay_frequency: v as PayFreq })}>
          ...
          <Field label="Rounding decimals">
            <Input type="number" min={0} max={6} value={form.rounding_decimals}
          ...
          <div className="col-span-2 flex justify-end">
            <Button type="submit" disabled={busy}>{busy ? "Saving…" : "Save defaults"}</Button>
          </div>
        </form>
      </CardContent>
    </Card>
...
function TaxBracketsPanel({
  countryCode, rows, onChange,
}: { countryCode: string; rows: TaxBracket[]; onChange: () => void }) {
  ...
  async function add(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("tax_brackets").insert({
      ...
      max_income: draft.max_income != null && (draft.max_income as unknown as string) !== "" ? Number(draft.max_income) : null,
  ...
  async function toggleActive(b: TaxBracket) {
    const { error } = await supabase.from("tax_brackets").update({ is_active: !b.is_active }).eq("id", b.id);
    if (error) return toast.error(error.message);
    onChange();
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Progressive income tax bands. Lower bracket order applies first.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <form onSubmit={add} className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <Field label="Order"><Input type="number" min={1} value={draft.bracket_order ?? 1} onChange={(e) => setDraft({ ...draft, bracket_order: Number(e.target.value) })} /></Field>
          <Field label="Effective from"><Input type="date" value={draft.effective_from ?? ""} onChange={(e) => setDraft({ ...draft, effective_from: e.target.value })} required /></Field>
          <Field label="Effective to"><Input type="date" value={draft.effective_to ?? ""} onChange={(e) => setDraft({ ...draft, effective_to: e.target.value || null })} /></Field>
          <Field label="Min income"><Input type="number" step="0.01" value={draft.min_income ?? 0} onChange={(e) => setDraft({ ...draft, min_income: Number(e.target.value) })} /></Field>
          <Field label="Max income (blank = ∞)"><Input type="number" step="0.01" value={draft.max_income ?? ""} onChange={(e) => setDraft({ ...draft, max_income: e.target.value === "" ? null : Number(e.target.value) })} /></Field>
          <Field label="Rate %"><Input type="number" step="0.001" min={0} max={100} value={draft.rate_percent ?? 0} onChange={(e) => setDraft({ ...draft, rate_percent: Number(e.target.value) })} required /></Field>
          <Field label="Fixed amount"><Input type="number" step="0.01" value={draft.fixed_amount ?? 0} onChange={(e) => setDraft({ ...draft, fixed_amount: Number(e.target.value) })} /></Field>
          <div className="md:col-span-4 flex justify-end">
            <Button type="submit" disabled={busy}>{busy ? "Adding…" : "Add bracket"}</Button>
          </div>
        </form>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>#</TableHead><TableHead>Name</TableHead><TableHead>Effective</TableHead>
              <TableHead>Income range</TableHead><TableHead>Rate</TableHead><TableHead>Fixed</TableHead>
              <TableHead>Status</TableHead><TableHead className="text-right">Actions</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {rows.length === 0 && <TableRow><TableCell colSpan={8} className="text-muted-foreground text-center py-8">No brackets yet.</TableCell></TableRow>}
            {rows.map((b) => (
              <TableRow key={b.id}>
                <TableCell>{b.bracket_order}</TableCell>
                <TableCell>{b.name}</TableCell>
                <TableCell className="text-xs">{b.effective_from}{b.effective_to ? ` → ${b.effective_to}` : ""}</TableCell>
                <TableCell className="text-xs">{b.min_income} – {b.max_income ?? "∞"}</TableCell>
                ...
                <TableCell className="text-right space-x-2">
                  <Button size="sm" variant="outline" onClick={() => toggleActive(b)}>{b.is_active ? "Disable" : "Enable"}</Button>
...
function ContributionsPanel({
  countryCode, rows, onChange,
}: { countryCode: string; rows: ContribRule[]; onChange: () => void }) {
  ...
  async function add(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("contribution_rules").insert({
      ...
      max_base: draft.max_base != null && (draft.max_base as unknown as string) !== "" ? Number(draft.max_base) : null,
      is_active: draft.is_active ?? true,
    });
    setBusy(false);
    if (error) return toast.error(error.message);
  ...
  async function toggleActive(c: ContribRule) {
    const { error } = await supabase.from("contribution_rules").update({ is_active: !c.is_active }).eq("id", c.id);
    if (error) return toast.error(error.message);
    onChange();
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Social security, pension, healthcare — anything with an employee/employer split.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <form onSubmit={add} className="grid grid-cols-2 gap-3 md:grid-cols-4">
          <Field label="Name"><Input value={draft.name ?? ""} onChange={(e) => setDraft({ ...draft, name: e.target.value })} required /></Field>
          ...
          <Field label="Effective to"><Input type="date" value={draft.effective_to ?? ""} onChange={(e) => setDraft({ ...draft, effective_to: e.target.value || null })} /></Field>
          <div />
          <Field label="Employee rate %"><Input type="number" step="0.001" min={0} max={100} value={draft.employee_rate_percent ?? 0} onChange={(e) => setDraft({ ...draft, employee_rate_percent: Number(e.target.value) })} /></Field>
          <Field label="Employer rate %"><Input type="number" step="0.001" min={0} max={100} value={draft.employer_rate_percent ?? 0} onChange={(e) => setDraft({ ...draft, employer_rate_percent: Number(e.target.value) })} /></Field>
          <Field label="Min base"><Input type="number" step="0.01" value={draft.min_base ?? 0} onChange={(e) => setDraft({ ...draft, min_base: Number(e.target.value) })} /></Field>
          <Field label="Max base (blank = ∞)"><Input type="number" step="0.01" value={draft.max_base ?? ""} onChange={(e) => setDraft({ ...draft, max_base: e.target.value === "" ? null : Number(e.target.value) })} /></Field>
          <div className="md:col-span-4 flex justify-end">
            <Button type="submit" disabled={busy}>{busy ? "Adding…" : "Add contribution"}</Button>
          </div>
        </form>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Name</TableHead><TableHead>Effective</TableHead>
              <TableHead>Employee</TableHead><TableHead>Employer</TableHead>
              <TableHead>Base</TableHead><TableHead>Status</TableHead>
              <TableHead className="text-right">Actions</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {rows.length === 0 && <TableRow><TableCell colSpan={7} className="text-muted-foreground text-center py-8">No contributions yet.</TableCell></TableRow>}
            {rows.map((c) => (
              <TableRow key={c.id}>
                <TableCell>{c.name}</TableCell>
                <TableCell className="text-xs">{c.effective_from}{c.effective_to ? ` → ${c.effective_to}` : ""}</TableCell>
                <TableCell>{c.employee_rate_percent}%</TableCell>
                <TableCell>{c.employer_rate_percent}%</TableCell>
                <TableCell className="text-xs">{c.min_base} – {c.max_base ?? "∞"}</TableCell>
                <TableCell><Badge variant={c.is_active ? "default" : "secondary"}>{c.is_active ? "active" : "inactive"}</Badge></TableCell>
                <TableCell className="text-right space-x-2">
                  <Button size="sm" variant="outline" onClick={() => toggleActive(c)}>{c.is_active ? "Disable" : "Enable"}</Button>
