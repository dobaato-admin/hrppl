...
export const Route = createFileRoute("/admin/kpi-kra")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse standard KPI and KRA presets and apply them to your tenant in one click." },
    ],
  }),
    ...
    <AppShell
      ...
      actions={
        <Link to="/admin/review-templates">
          <Button variant="outline" size="sm">All review templates</Button>
        </Link>
      }
    >
      ...
      <section className="mx-auto max-w-6xl px-6 py-6 space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Filter by industry, kind, role, or any keyword in the preset (including KPI item labels).</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <div className="text-xs text-muted-foreground">{presets.length} preset{presets.length === 1 ? "" : "s"} found</div>
              ...
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <CardDescription className="text-xs">{p.role} · {p.industry}</CardDescription>
                  </div>
                  <Badge variant={p.kind === "kpi" ? "default" : "secondary"} className="uppercase">{p.kind}</Badge>
                </div>
              </CardHeader>
              <CardContent className="flex-1 space-y-2">
                ...
                <div className="flex flex-wrap gap-1">
                  {p.competencies.slice(0, 4).map((c) => (
                    <Badge key={c.id} variant="outline" className="text-[10px] font-normal">{c.label}</Badge>
                  ))}
                  {p.competencies.length > 4 && (
                    <Badge variant="outline" className="text-[10px] font-normal">+{p.competencies.length - 4} more</Badge>
                  )}
                </div>
                <div className="pt-2">
          ...
          {presets.length === 0 && (
            <Card className="md:col-span-2">
              <CardContent className="py-10 text-center text-sm text-muted-foreground">
                No KPI or KRA presets match your filters. Try clearing the search or industry filter.
              </CardContent>
            </Card>
          )}
        </div>
