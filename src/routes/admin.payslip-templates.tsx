...
const SANITIZE_CONFIG = {
  ALLOWED_TAGS: ["h1","h2","h3","h4","h5","h6","p","span","strong","em","b","i","u","br","hr","table","thead","tbody","tr","td","th","ul","ol","li","div"],
...
const LOCALES = ["en-US", "en-GB", "en-AU", "fr-FR", "de-DE", "es-ES", "pt-BR", "ja-JP", "zh-CN", "ar-AE"];
...
function PayslipTemplatesPage() {
  ...
  const canEdit = roles.includes("super_admin") || roles.includes("regional_admin") || roles.includes("org_admin");
  ...
  const [auditEntries, setAuditEntries] = useState<Array<{
    id: string; action: string; created_at: string; actor_id: string | null;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (roles.includes("org_admin") && !roles.includes("super_admin") && !roles.includes("regional_admin")) {
  ...
  async function loadTemplates(cc: string) {
    const { data } = await supabase.from("payslip_templates").select("*")
      .eq("country_code", cc).order("created_at", { ascending: false });
    const list = (data ?? []) as unknown as Template[];
    setTemplates(list);
    setSelectedId(list[0]?.id ?? "");
  }

  useEffect(() => { if (country) loadTemplates(country); }, [country]);

  useEffect(() => {
    if (!selectedId) { setLineItems([]); return; }
    (async () => {
      const { data } = await supabase.from("payslip_line_items").select("*")
        .eq("template_id", selectedId).order("sort_order");
  ...
  async function createTemplate() {
    ...
    setSaving(true);
    const { data, error } = await supabase.from("payslip_templates").insert({
    ...
    await logAudit("payslip_template.create", tpl.id, { name: tpl.name, country_code: tpl.country_code });
  ...
  async function saveTemplate(patch: Partial<Template>) {
    ...
    setSaving(false);
    if (error) return toast.error(error.message);
    setTemplates((prev) => prev.map((t) => t.id === selected.id ? { ...t, ...patch } as Template : t));
    await logAudit("payslip_template.draft.edit", selected.id, { fields: Object.keys(patch), version: selected.version });
    loadAudit(selected);
    toast.success("Saved");
  }
  ...
  async function deleteTemplate() {
    ...
    const snapshot = { name: selected.name, version: selected.version, country_code: selected.country_code };
  ...
  async function addLineItem() {
    ...
    if (error) return toast.error(error.message);
    const { data } = await supabase.from("payslip_line_items").select("*")
      .eq("template_id", selected.id).order("sort_order");
    setLineItems((data ?? []) as unknown as LineItem[]);
  ...
  async function updateLineItem(id: string, patch: Partial<LineItem>) {
    ...
    if (error) return toast.error(error.message);
    await logAudit("payslip_template.draft.edit", selected.id, { change: "line_item.update", line_item_id: id, fields: Object.keys(patch) });
    loadAudit(selected);
  }
  ...
  async function removeLineItem(id: string) {
    ...
    if (error) return toast.error(error.message);
    await logAudit("payslip_template.draft.edit", selected.id, { change: "line_item.remove", line_item_id: id });
    loadAudit(selected);
  }
  ...
  async function cloneAsDraft() {
    if (!selected) return;
    const { data, error } = await supabase.rpc("clone_payslip_template", { _template_id: selected.id });
    if (error) return toast.error(error.message);
    if (typeof data === "string") {
      await logAudit("payslip_template.clone", data, { from_template_id: selected.id, from_version: selected.version });
    }
  ...
  async function publishDraft() {
    ...
    const priorPublished = templates.find(
      (t) => (t.parent_template_id ?? t.id) === root && t.status === "published" && t.id !== selected.id,
    );
  ...
  async function loadAudit(tpl: Template) {
    ...
    if (actorIds.length) {
      ...
      emails = Object.fromEntries((profs ?? []).map((p) => [p.id as string, (p.email as string) ?? ""]));
    }
    setAuditEntries(rows.map((r) => ({ ...r, actor_email: r.actor_id ? emails[r.actor_id] ?? null : null })));
  }

  useEffect(() => {
    if (selected) loadAudit(selected); else setAuditEntries([]);
      ...
      <header className="border-b border-border bg-card">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Configurable line items and localized formatting per country</p>
          </div>
          <Link to="/dashboard">
          ...
          <CardContent className="flex flex-wrap items-end gap-4">
              ...
              <Select value={country} onValueChange={setCountry}>
                ...
                <SelectContent>
                  {visibleCountries.map((c) => (
                    <SelectItem key={c.code} value={c.code}>{c.name} ({c.currency_code})</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div className="w-72 space-y-2">
              ...
              <Select value={selectedId} onValueChange={setSelectedId}>
                <SelectTrigger><SelectValue placeholder={templates.length ? "Select…" : "No templates yet"} /></SelectTrigger>
                <SelectContent>
                  {templates.map((t) => (
            ...
            <Button onClick={createTemplate} disabled={!country || saving}>New template</Button>
            {selected && selected.status === "draft" && (
              <Button onClick={publishDraft}>Publish…</Button>
            )}
            {selected && (
              <Button variant="outline" onClick={cloneAsDraft}>New version</Button>
            )}
            {selected && selected.status === "draft" && (
              <Button variant="destructive" onClick={deleteTemplate}>Delete draft</Button>
            )}
          </CardContent>
        </Card>
        ...
        {selected && familyVersions.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Published versions are immutable so historical payslips keep referencing them.</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {familyVersions.map((v) => (
                    <TableRow key={v.id} className={v.id === selected.id ? "bg-muted/40" : undefined}>
                      ...
                      <TableCell className="text-xs text-muted-foreground">{v.published_at?.slice(0, 10) ?? "—"}</TableCell>
                      <TableCell>
                        {v.id !== selected.id && (
                          <Button size="sm" variant="ghost" onClick={() => setSelectedId(v.id)}>Open</Button>
                        )}
                      </TableCell>
                    </TableRow>
                  ))}
        ...
        {selected && roles.includes("super_admin") && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Who changed what, across every version in this template family.</CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    {auditEntries.map((a) => {
                      ...
                      return (
                        <TableRow key={a.id}>
                          ...
                          <TableCell><Badge variant="outline">{a.action.replace("payslip_template.", "")}</Badge></TableCell>
                          <TableCell className="text-xs">{tpl ? `v${tpl.version}` : "—"}</TableCell>
        ...
        {selected && (
          ...
          {isLocked && (
            <div className="pointer-events-auto mb-4 rounded-md border border-amber-500/40 bg-amber-500/10 px-4 py-2 text-sm">
              This version is <strong>{selected.status}</strong> and read-only. Click "New version" above to make changes.
            </div>
          )}
                ...
                <CardContent className="grid gap-4 pt-6 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input value={selected.name}
                      onChange={(e) => setTemplates((p) => p.map((t) => t.id === selected.id ? { ...t, name: e.target.value } : t))}
                      onBlur={(e) => saveTemplate({ name: e.target.value })} />
                  </div>
                  <div className="space-y-2">
                    <Label>Currency</Label>
                    <Input value={selected.currency_code}
                      onChange={(e) => setTemplates((p) => p.map((t) => t.id === selected.id ? { ...t, currency_code: e.target.value.toUpperCase() } : t))}
                      onBlur={(e) => saveTemplate({ currency_code: e.target.value.toUpperCase() })} />
                  </div>
                  <div className="flex items-center justify-between rounded-md border p-3">
                    <div>
                      <Label>Default template</Label>
                      <p className="text-xs text-muted-foreground">Used by tenants in this country</p>
                    </div>
                    <Switch checked={selected.is_default}
                      onCheckedChange={(v) => saveTemplate({ is_default: v })} />
                  </div>
                  <div className="flex items-center justify-between rounded-md border p-3">
                    <div><Label>Active</Label></div>
                    <Switch checked={selected.is_active}
                      onCheckedChange={(v) => saveTemplate({ is_active: v })} />
                  </div>
                  <div className="flex items-center justify-between rounded-md border p-3">
                    <div><Label>Show employer contributions</Label></div>
                    <Switch checked={selected.show_employer_contributions}
                      onCheckedChange={(v) => saveTemplate({ show_employer_contributions: v })} />
                  </div>
                  <div className="flex items-center justify-between rounded-md border p-3">
                    <div><Label>Show year-to-date totals</Label></div>
                    <Switch checked={selected.show_ytd}
            ...
            <TabsContent value="formatting">
              <Card>
                <CardContent className="grid gap-4 pt-6 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label>Locale</Label>
                    <Select value={selected.locale} onValueChange={(v) => saveTemplate({ locale: v })}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={selected.date_format} onValueChange={(v) => saveTemplate({ date_format: v })}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                        {DATE_FORMATS.map((l) => <SelectItem key={l} value={l}>{l}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  <div className="space-y-2">
                    <Label>Currency position</Label>
                    <Select value={selected.currency_position} onValueChange={(v) => saveTemplate({ currency_position: v })}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="number" min={0} max={6}
                      ...
                      onChange={(e) => {
                        const nf = { ...selected.number_format, decimals: Number(e.target.value) };
                        setTemplates((p) => p.map((t) => t.id === selected.id ? { ...t, number_format: nf } : t));
                      }}
                      onBlur={() => saveTemplate({ number_format: selected.number_format })} />
                  </div>
                  <div className="space-y-2">
                    <Label>Decimal separator</Label>
                    <Input maxLength={1} value={selected.number_format.decimal}
                      onChange={(e) => {
                  ...
                  <div className="space-y-2">
                    ...
                    <Input maxLength={1} value={selected.number_format.thousands}
                      onChange={(e) => {
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Earnings, deductions, and contributions in payslip order.</CardDescription>
                  </div>
                ...
                <CardContent>
                  <Table>
                    ...
                    <TableBody>
                      {lineItems.length === 0 ? (
                        <TableRow><TableCell colSpan={9} className="text-muted-foreground">No line items yet.</TableCell></TableRow>
                      ) : lineItems.map((l, idx) => (
                        <TableRow key={l.id}>
                          <TableCell>
                            <Input type="number" className="w-16" value={l.sort_order}
                              onChange={(e) => updateLineItem(l.id, { sort_order: Number(e.target.value) })} />
                          </TableCell>
                          <TableCell>
                            <Input className="w-28 font-mono text-xs" value={l.code}
                              onChange={(e) => setLineItems((p) => p.map((x) => x.id === l.id ? { ...x, code: e.target.value } : x))}
                              onBlur={(e) => updateLineItem(l.id, { code: e.target.value })} />
                          </TableCell>
                          <TableCell>
                            <Input value={l.label}
                              onChange={(e) => setLineItems((p) => p.map((x) => x.id === l.id ? { ...x, label: e.target.value } : x))}
                              onBlur={(e) => updateLineItem(l.id, { label: e.target.value })} />
                          </TableCell>
                          <TableCell>
                            <Select value={l.category} onValueChange={(v) => updateLineItem(l.id, { category: v })}>
                          ...
                          <TableCell>
                            <Select value={l.calc_type} onValueChange={(v) => updateLineItem(l.id, { calc_type: v })}>
                              <SelectTrigger className="w-36"><SelectValue /></SelectTrigger>
                              <SelectContent>
                                {CALC_TYPES.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                              </SelectContent>
                            </Select>
                          </TableCell>
                          <TableCell>
                            {l.calc_type === "percent_of_base" ? (
                              <Input type="number" step="0.01" className="w-24" value={l.rate ?? ""}
                                onChange={(e) => setLineItems((p) => p.map((x) => x.id === l.id ? { ...x, rate: Number(e.target.value) } : x))}
                                onBlur={(e) => updateLineItem(l.id, { rate: Number(e.target.value) })} />
                            ) : l.calc_type === "formula" ? (
                              <Input className="w-44 font-mono text-xs" placeholder="base * 0.05" value={l.formula ?? ""}
                                onChange={(e) => setLineItems((p) => p.map((x) => x.id === l.id ? { ...x, formula: e.target.value } : x))}
                                onBlur={(e) => updateLineItem(l.id, { formula: e.target.value })} />
                            ) : (
                              <Input type="number" step="0.01" className="w-24" value={l.rate ?? ""}
                                onChange={(e) => setLineItems((p) => p.map((x) => x.id === l.id ? { ...x, rate: Number(e.target.value) } : x))}
                                onBlur={(e) => updateLineItem(l.id, { rate: Number(e.target.value) })} />
                          ...
                          <TableCell>
                            <Switch checked={l.is_taxable} onCheckedChange={(v) => updateLineItem(l.id, { is_taxable: v })} />
                          </TableCell>
                          <TableCell>
                            <Switch checked={l.is_visible} onCheckedChange={(v) => updateLineItem(l.id, { is_visible: v })} />
                          </TableCell>
                          <TableCell>
                            <Button variant="ghost" size="sm" onClick={() => removeLineItem(l.id)}>Remove</Button>
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
              ...
              <Card>
                <CardContent className="grid gap-4 pt-6">
                  <div className="space-y-2">
                    ...
                    <Textarea rows={6} value={selected.header_html ?? ""}
                      onChange={(e) => setTemplates((p) => p.map((t) => t.id === selected.id ? { ...t, header_html: e.target.value } : t))}
                      onBlur={(e) => saveTemplate({ header_html: e.target.value })}
                      placeholder="<h2>{{company_name}}</h2><p>Pay period: {{period}}</p>" />
                  </div>
                  <div className="space-y-2">
                    <Label>Footer HTML</Label>
                    <Textarea rows={4} value={selected.footer_html ?? ""}
                      onChange={(e) => setTemplates((p) => p.map((t) => t.id === selected.id ? { ...t, footer_html: e.target.value } : t))}
              ...
              <Card>
                <CardContent className="pt-6">
                  <div className="rounded-md border bg-card p-6">
                    <div className="border-b pb-3" dangerouslySetInnerHTML={{ __html: sanitizeHtml(selected.header_html ?? `<h2 class='text-xl font-semibold'>${selected.name}</h2>`) }} />
                    <div className="mt-4 flex justify-between text-sm text-muted-foreground">
                      <span>Pay date: {formattedDate}</span>
                      <span>{selected.country_code} · {selected.currency_code}</span>
                    </div>
                    <table className="mt-4 w-full text-sm">
                      <thead><tr className="border-b text-left"><th className="py-1">Code</th><th>Description</th><th className="text-right">Amount</th></tr></thead>
                      <tbody>
                        {lineItems.filter((l) => l.is_visible).map((l) => (
                          <tr key={l.id} className="border-b">
                            <td className="py-1 font-mono text-xs">{l.code}</td>
                            <td>{l.label} <Badge variant="outline" className="ml-1 text-[10px]">{l.category}</Badge></td>
                            <td className="text-right">{formatted}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    <div className="mt-4 border-t pt-2 text-xs text-muted-foreground" dangerouslySetInnerHTML={{ __html: sanitizeHtml(selected.footer_html ?? "") }} />
                  </div>
                </CardContent>
              </Card>
            </TabsContent>
