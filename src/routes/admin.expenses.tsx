...
function CategoriesTab() {
  ...
  function blankCategory() {
    return {
      id: undefined, name: "", code: "", description: "",
      requires_receipt: true, is_active: true,
      max_amount: "", daily_limit: "", monthly_limit: "",
      tax_rate: "", tax_code: "",
    };
  }
  ...
  function openNew() { setForm(blankCategory()); setOpen(true); }
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>Per-tenant categories, receipt rules, spending caps and tax rates.</CardDescription>
        </div>
      ...
      <CardContent>
        <Table>
          ...
          <TableBody>
            {rows.length === 0 ? (
              <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-6">No categories yet</TableCell></TableRow>
            ) : rows.map((c) => (
              <TableRow key={c.id}>
                <TableCell className="font-medium">{c.name}</TableCell>
                <TableCell className="text-xs text-muted-foreground">{c.code ?? "—"}</TableCell>
                <TableCell className="text-xs">
                  {c.tax_rate ? `${(Number(c.tax_rate) * 100).toFixed(2)}%${c.tax_code ? ` (${c.tax_code})` : ""}` : "—"}
                </TableCell>
                <TableCell className="font-mono text-xs">{c.max_amount != null ? Number(c.max_amount).toFixed(2) : "—"}</TableCell>
                <TableCell><Badge variant={c.requires_receipt ? "default" : "secondary"}>{c.requires_receipt ? "Required" : "Optional"}</Badge></TableCell>
                <TableCell><Badge variant={c.is_active ? "default" : "secondary"}>{c.is_active ? "Active" : "Inactive"}</Badge></TableCell>
                <TableCell className="text-right">
                  <Button size="sm" variant="ghost" onClick={() => openEdit(c)}><Pencil className="h-3.5 w-3.5" /></Button>
                  {c.is_active ? <Button size="sm" variant="ghost" onClick={() => onDelete(c.id)}><Trash2 className="h-3.5 w-3.5" /></Button> : null}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-xl">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div>
              <Label>Code</Label>
              <Input value={form.code} maxLength={40} onChange={(e) => setForm({ ...form, code: e.target.value })} />
            </div>
            <div>
              <Label>Tax code</Label>
              <Input value={form.tax_code} placeholder="GST / VAT / SST…" maxLength={40} onChange={(e) => setForm({ ...form, tax_code: e.target.value })} />
            </div>
            <div>
              <Label>Tax rate</Label>
              <Input type="number" step="0.0001" min={0} max={1} placeholder="0.10 = 10%" value={form.tax_rate} onChange={(e) => setForm({ ...form, tax_rate: e.target.value })} />
            </div>
            <div>
              <Label>Per-line limit</Label>
              <Input type="number" step="0.01" min={0} value={form.max_amount} onChange={(e) => setForm({ ...form, max_amount: e.target.value })} />
            </div>
            <div>
              <Label>Daily limit</Label>
              <Input type="number" step="0.01" min={0} value={form.daily_limit} onChange={(e) => setForm({ ...form, daily_limit: e.target.value })} />
            </div>
            <div>
              <Label>Monthly limit</Label>
              <Input type="number" step="0.01" min={0} value={form.monthly_limit} onChange={(e) => setForm({ ...form, monthly_limit: e.target.value })} />
            </div>
            <div className="md:col-span-2">
...
function RulesTab() {
  ...
  function blank() {
    return { id: undefined, department_id: null, min_amount: "0", max_amount: "", approver_id: "", priority: 100, is_active: true, notes: "" };
  }
  ...
  async function submit() {
    ...
    try { await save({ data: payload }); toast.success("Saved"); setOpen(false); await refresh(); }
    catch (e: any) { toast.error(e.message); }
  }
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            On submit, claims route to the highest-priority active rule whose department and amount band match.
            Multiple matches mean multiple approvers can act.
          </CardDescription>
        </div>
      ...
      <CardContent>
        <Table>
          ...
          <TableBody>
            {rows.length === 0 ? (
              <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-6">No rules yet — without a rule, claims are visible only to org admins.</TableCell></TableRow>
            ) : rows.map((r) => {
              const approver = employees.find((e) => e.id === r.approver_id);
              return (
                <TableRow key={r.id}>
                  <TableCell className="font-mono text-xs">{r.priority}</TableCell>
                  <TableCell>{r.departments?.name ?? <span className="text-muted-foreground">All</span>}</TableCell>
                  <TableCell className="font-mono text-xs">
                    {Number(r.min_amount).toFixed(2)} – {r.max_amount == null ? "∞" : Number(r.max_amount).toFixed(2)}
                  </TableCell>
                  <TableCell>{approver ? `${approver.first_name} ${approver.last_name}` : <span className="text-muted-foreground">—</span>}</TableCell>
                  <TableCell><Badge variant={r.is_active ? "default" : "secondary"}>{r.is_active ? "Active" : "Inactive"}</Badge></TableCell>
                  <TableCell className="text-right">
                    <Button size="sm" variant="ghost" onClick={() => openEdit(r)}><Pencil className="h-3.5 w-3.5" /></Button>
                    <Button size="sm" variant="ghost" onClick={() => onDelete(r.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                  </TableCell>
                </TableRow>
              );
            })}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{form.id ? "Edit rule" : "New approval rule"}</DialogTitle></DialogHeader>
          <div className="grid gap-3 md:grid-cols-2">
            <div className="md:col-span-2">
              <Label>Department (optional)</Label>
              <Select value={form.department_id ?? "__all"} onValueChange={(v) => setForm({ ...form, department_id: v === "__all" ? null : v })}>
            ...
            <div>
              ...
              <Input type="number" step="0.01" min={0} value={form.min_amount} onChange={(e) => setForm({ ...form, min_amount: e.target.value })} />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <Select value={form.approver_id} onValueChange={(v) => setForm({ ...form, approver_id: v })}>
                ...
                <SelectContent>
                  {employees.map((e) => <SelectItem key={e.id} value={e.id}>{e.first_name} {e.last_name} {e.job_title ? `· ${e.job_title}` : ""}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              <Label>Notes</Label>
              <Input value={form.notes} maxLength={500} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button>
