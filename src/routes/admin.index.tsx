...
interface Country { code: string; name: string; region_code: string; currency_code: string }
interface Tenant { id: string; name: string; slug: string; country_code: string; currency_code: string; status: string; contact_email: string; plan: string | null; created_at: string }
interface RegAdmin { user_id: string; email: string | null; full_name: string | null; countries: string[] }
interface AuditRow { id: string; actor_id: string | null; action: string; entity_type: string; entity_id: string | null; metadata: any; created_at: string }
...
function AdminPage() {
  ...
  const [actorMap, setActorMap] = useState<Record<string, { email: string | null; full_name: string | null }>>({});
  ...
  async function refresh() {
    ...
    if (actorIds.length) {
      const { data: ap } = await supabase.from("profiles").select("id,email,full_name").in("id", actorIds);
      const m: Record<string, { email: string | null; full_name: string | null }> = {};
      (ap ?? []).forEach((p: any) => { m[p.id] = { email: p.email, full_name: p.full_name }; });
      setActorMap(m);
    }
  }
  ...
  async function grantRegional(e: React.FormEvent) {
    ...
    if (pe || !prof) { setWorking(false); return toast.error("User must sign up first with that email"); }
    const { error: re } = await supabase.from("user_roles")
      .insert({ user_id: prof.id, role: "regional_admin" });
    if (re && !re.message.includes("duplicate")) { setWorking(false); return toast.error(re.message); }
    const { error: se } = await supabase.from("role_scope")
  ...
  async function removeRegionalCountry(userId: string, code: string) {
    ...
    const { error } = await supabase.from("role_scope").delete().eq("user_id", userId).eq("country_code", code);
  ...
  async function revokeRegional(userId: string) {
    if (!confirm("Revoke regional_admin from this user? All country scopes will also be removed.")) return;
    setWorking(true);
    const { error: e1 } = await supabase.from("role_scope").delete().eq("user_id", userId);
    if (e1) { setWorking(false); return toast.error(e1.message); }
    const { error: e2 } = await supabase.from("user_roles").delete().eq("user_id", userId).eq("role", "regional_admin");
    setWorking(false);
  ...
  const entityTypes = useMemo(() => Array.from(new Set(audit.map((r) => r.entity_type))).sort(), [audit]);
    ...
    <AppShell
      ...
      actions={
        <div className="hidden md:flex items-center gap-2">
          ...
          <Link to="/admin/payroll-settings"><Button size="sm" variant="outline">Payroll Settings</Button></Link>
          <Link to="/admin/payslip-templates"><Button size="sm">Payslip Templates</Button></Link>
        </div>
      }
    >
      <section className="mx-auto grid max-w-7xl gap-6 px-6 py-8">
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <KpiTile label="Tenants" value={tenants.length} tone="primary" icon={Building2} hint={`${activeTenants} active`} />
          <KpiTile label="Suspended" value={suspendedTenants} tone={suspendedTenants > 0 ? "stuck" : "done"} icon={Building2} />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>User must already have signed up. Bind one country at a time.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={grantRegional} className="grid gap-4 md:grid-cols-[1fr_200px_auto] md:items-end">
              ...
              <Button type="submit" disabled={working}>Grant</Button>
            </form>
          </CardContent>
        </Card>
        ...
        <Card className="overflow-hidden">
          ...
          <CardHeader>
            ...
            <CardDescription>Click a country chip to remove that scope, or revoke the role entirely.</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {regionals.length === 0 ? (
                  <TableRow><TableCell colSpan={4} className="text-muted-foreground">No regional admins yet.</TableCell></TableRow>
                ) : regionals.map((r) => (
                  <TableRow key={r.user_id}>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        {r.countries.length === 0 && <span className="text-xs text-muted-foreground">No scopes</span>}
                        {r.countries.map((c) => (
                          <button
                            key={c}
                          ...
                          >
                            <Badge variant="outline" className="cursor-pointer group-hover:bg-destructive group-hover:text-destructive-foreground group-hover:border-destructive">
                              {c}
                    ...
                    <TableCell className="text-right">
                      <Button size="sm" variant="ghost" disabled={working} onClick={() => revokeRegional(r.user_id)}>
                        Revoke
                      </Button>
                    </TableCell>
                  </TableRow>
        ...
        <Card className="overflow-hidden">
          ...
          <CardHeader className="flex flex-row items-center justify-between gap-3">
            <div>
              ...
              <CardDescription>Every organization across all regions. Use the actions to suspend or reactivate.</CardDescription>
            </div>
            <div className="relative w-64">
              <Search className="absolute left-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
              <Input value={tenantSearch} onChange={(e) => setTenantSearch(e.target.value)} placeholder="Search tenants…" className="pl-7" />
            </div>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Status</TableHead><TableHead>Contact</TableHead>
                  <TableHead className="text-right">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredTenants.length === 0 ? (
                  <TableRow><TableCell colSpan={7} className="text-muted-foreground">No tenants.</TableCell></TableRow>
                  ...
                  return (
                    <TableRow key={t.id}>
                      ...
                      <TableCell className="text-right space-x-1">
                        {t.status !== "active" && (
                          <Button size="sm" variant="outline" onClick={() => setTenantStatus(t.id, "active")}>Activate</Button>
                        )}
                        {t.status === "active" && (
                          <Button size="sm" variant="outline" onClick={() => setTenantStatus(t.id, "suspended")}>Suspend</Button>
                        )}
                        {t.status !== "cancelled" && (
                          <Button size="sm" variant="ghost" onClick={() => setTenantStatus(t.id, "cancelled")}>Cancel</Button>
                        )}
                      </TableCell>
                    </TableRow>
                  );
            ...
            <Table>
              ...
              <TableBody>
                {filteredAudit.map((r) => {
                  ...
                  return (
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="text-xs">{actor?.email ?? actor?.full_name ?? (r.actor_id ? r.actor_id.slice(0, 8) + "…" : "system")}</TableCell>
                ...
                {filteredAudit.length === 0 && (
                  <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground py-6">No events.</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </CardContent>
