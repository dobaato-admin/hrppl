...
function OrgExpensesPage() {
  ...
  async function saveCat() {
    ...
    await upCat({
      data: { ...editCat, max_amount: editCat.max_amount ? Number(editCat.max_amount) : null },
    });
    toast.success("Saved");
    setCatOpen(false);
    setEditCat({ name: "", requires_receipt: true, is_active: true });
    await refreshCats();
  }
  ...
  function presetAlreadyAdded(p: ExpenseCategoryPreset) {
    ...
    return categories.some(
      (c) => (c.code ?? "").toLowerCase() === codeLc || (c.name ?? "").toLowerCase() === nameLc,
    );
  }
  ...
  return (
    <AppShell
      ...
      subtitle="Review, approve and reimburse staff expenses"
    >
      <div className="p-4 md:p-6">
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList>
            <TabsTrigger value="queue">
              <ReceiptText className="h-4 w-4 mr-1" /> Claims
            </TabsTrigger>
          ...
          <TabsContent value="categories" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Click a preset to add it instantly with sensible defaults. Use "Customize" to
                  tweak the receipt rule, max amount or code before saving.
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Your tenant's active categories — edit defaults or add a custom one.
                  </CardDescription>
                </div>
                <Button
                  onClick={() => {
      ...
      <Dialog open={!!drawer} onOpenChange={(o) => !o && setDrawer(null)}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="text-sm text-muted-foreground">
                  {drawer.claim.employees?.first_name} {drawer.claim.employees?.last_name} •{" "}
                  {drawer.claim.employees?.job_title}
                </div>
                <Badge className={STATUS_TONE[drawer.claim.status] + " border-0 capitalize"}>
                  {drawer.claim.status}
                </Badge>
              </div>
              <div className="text-2xl font-display">
                {drawer.claim.currency} {Number(drawer.claim.total_amount).toFixed(2)}
              </div>
              {drawer.claim.description && (
                <p className="text-sm text-muted-foreground">{drawer.claim.description}</p>
              )}
                      ...
                      <TableCell>
                        {l.receipt_path ? (
                          <Button
                            variant="link"
                            size="sm"
                            onClick={() => viewReceipt(l.receipt_path)}
                          >
                            View
                          </Button>
                        ) : (
              ...
              {drawer.claim.status === "recommended" && drawer.claim.recommended_at && (
                <div className="rounded-md border border-status-working/30 bg-status-working/5 p-3 text-sm">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {new Date(drawer.claim.recommended_at).toLocaleString()}
                  </div>
                  {drawer.claim.recommendation_note && (
                    <div className="mt-1">{drawer.claim.recommendation_note}</div>
                  )}
                </div>
              )}
                  ...
                  <ol className="space-y-2 border-l border-border pl-4">
                    {drawer.approvals.map((a: any) => (
                      <li key={a.id} className="text-sm">
                        ...
                        {a.comment && (
                          <div className="text-muted-foreground text-xs mt-0.5">{a.comment}</div>
                        )}
                      </li>
                    ))}
                  </ol>
          ...
          <DialogFooter>
            {(drawer?.claim?.status === "submitted" || drawer?.claim?.status === "recommended") && (
              <>
                <Button variant="outline" onClick={() => doDecide("reject")}>
                  <X className="h-4 w-4 mr-1" /> Reject
                </Button>
                <Button onClick={() => doDecide("approve")}>
                  <Check className="h-4 w-4 mr-1" /> Approve
                </Button>
              </>
            )}
            {drawer?.claim?.status === "approved" && (
              <Button onClick={() => doDecide("pay")}>
          ...
          <div className="space-y-3">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                ...
                <Input
                  value={editCat.code ?? ""}
                  onChange={(e) => setEditCat({ ...editCat, code: e.target.value })}
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  step="0.01"
                  value={editCat.max_amount ?? ""}
                  onChange={(e) => setEditCat({ ...editCat, max_amount: e.target.value })}
                />
              </div>
            </div>
            <label className="flex items-center gap-2 text-sm">
              <Checkbox
                checked={editCat.requires_receipt}
                onCheckedChange={(v) => setEditCat({ ...editCat, requires_receipt: !!v })}
              />{" "}
              Requires receipt
            </label>
            <label className="flex items-center gap-2 text-sm">
              <Checkbox
                ...
                onCheckedChange={(v) => setEditCat({ ...editCat, is_active: !!v })}
              />{" "}
              Active
            </label>
          </div>
